...
当前原创规则契约位于 [`tests/fixtures/contract-v95/scenarios`](tests/fixtures/contract-v95/scenarios)，由 `rfb-contract` 在所有平台运行；`contract-v1` 至 `contract-v94` 作为历史基准保留。
...
cargo run -p rfb-contract -- validate-policy tests/fixtures/contract-v95/baseline-policy.json
```

当前 308 个原创 contract fixtures、自动协议生成、原创内容包、ASCII glyph atlas、图片 tileset manifest、缺失资源回退和 Windows Tauri 端到端测试已经建立。桌面 E2E 可用以下命令运行：
