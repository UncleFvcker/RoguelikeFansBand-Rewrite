...
当前原创规则契约位于 [`tests/fixtures/contract-v17/scenarios`](tests/fixtures/contract-v17/scenarios)，由 `rfb-contract` 在所有平台运行；`contract-v1` 至 `contract-v16` 作为历史基准保留。
...
协议 1.17 / contract-v17 已建立内容驱动的弹药破损、未命中实体时的无 RNG 回收，以及保留/拆分实例 ID 的权威落地事务；此前的方向/格子/实体目标规格和前端目标模式继续使用。active baseline 共 54 个 exact fixtures。原创内容包为 1.12.0；存档继续为 v1，state hash Schema 继续为 v9。完整边界见 [Contract v17 说明](design/contract-v17-ammunition-recovery.md)。

下一规则切片建立投掷攻击 profile、重量射程与命中/伤害；鼠标点选、路径预览和投射物动画留在后续前端切片。
...
cargo run -p rfb-contract -- validate-policy tests/fixtures/contract-v17/baseline-policy.json
```

当前 54 个原创 contract fixtures、自动协议生成、原创内容包、ASCII glyph atlas、图片 tileset manifest、缺失资源回退和 Windows Tauri 端到端测试已经建立。桌面 E2E 可用以下命令运行：
