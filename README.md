...
- [Contract v11：结构化伤害与死亡事件](design/contract-v11-structured-damage-events.md)
...
当前原创规则契约位于 [`tests/fixtures/contract-v11/scenarios`](tests/fixtures/contract-v11/scenarios)，由 `rfb-contract` 在所有平台运行；`contract-v1` 至 `contract-v10` 作为历史基准保留。
...
协议 1.11 / contract-v11 已为所有伤害与死亡事件增加类型化 outcome，明确记录原始伤害、物理减伤、抗性调整、最终伤害、伤害类型和抗性等级；普通、弱点、抗性、强抗、免疫及致死结算均进入 exact fixtures。存档、内容包和 state hash 没有变化。完整边界见 [Contract v11 说明](design/contract-v11-structured-damage-events.md)。

contract-v11 接下来继续补齐酸、电、冷、毒入口和带来源的派生属性/检定接口，再实现眩晕与恐惧；随后由 contract-v12 承接武器 `AttackProfile`、玩家攻击次数和怪物多 blow，避免继续复用已经落地的版本编号。
...
当前 41 个原创 contract fixtures、自动协议生成、原创内容包、ASCII glyph atlas、图片 tileset manifest、缺失资源回退和 Windows Tauri 端到端测试已经建立。桌面 E2E 可用以下命令运行：
