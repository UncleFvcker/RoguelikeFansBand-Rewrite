...
当前原创规则契约位于 [`tests/fixtures/contract-v12/scenarios`](tests/fixtures/contract-v12/scenarios)，由 `rfb-contract` 在所有平台运行；`contract-v1` 至 `contract-v11` 作为历史基准保留。
...
协议 1.12 / contract-v12 已建立内容驱动的武器 `AttackProfile`、命中/伤害修正和玩家稳定多段近战；目标死亡会立即中断剩余攻击，恐惧仍按整次行动检定一次。active baseline 共 48 个 exact fixtures。原创内容包为 1.8.0，新增双击回声刃；存档继续为 v1，state hash Schema 继续为 v9。完整边界见 [Contract v12 说明](design/contract-v12-weapon-attack-profile.md)。

contract-v12 的下一切片承接怪物 `MeleeRoutine` / 多 blow；远程与投掷留给后续 projectile contract。
...
cargo run -p rfb-contract -- validate-policy tests/fixtures/contract-v12/baseline-policy.json
```

当前 48 个原创 contract fixtures、自动协议生成、原创内容包、ASCII glyph atlas、图片 tileset manifest、缺失资源回退和 Windows Tauri 端到端测试已经建立。桌面 E2E 可用以下命令运行：
