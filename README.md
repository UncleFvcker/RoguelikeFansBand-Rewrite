...
当前原创规则契约位于 [`tests/fixtures/contract-v18/scenarios`](tests/fixtures/contract-v18/scenarios)，由 `rfb-contract` 在所有平台运行；`contract-v1` 至 `contract-v17` 作为历史基准保留。
...
协议 1.18 / contract-v18 已建立整数物品重量、内容驱动的投掷 profile、重量射程、独立命中检定以及复用 `DamagePacket` 的伤害/死亡结算。active baseline 共 55 个 exact fixtures。原创内容包为 1.13.0；存档继续为 v1，state hash Schema 继续为 v9。完整边界见 [Contract v18 说明](design/contract-v18-thrown-attacks.md)。

阶段 C 的普通近战、射击和投掷纵向闭环已经建立。下一规则切片进入阶段 D，优先利用现有重量建立背包总重量与容量；鼠标点选、路径预览和投射物动画留在后续前端切片。
...
cargo run -p rfb-contract -- validate-policy tests/fixtures/contract-v18/baseline-policy.json
```

当前 55 个原创 contract fixtures、自动协议生成、原创内容包、ASCII glyph atlas、图片 tileset manifest、缺失资源回退和 Windows Tauri 端到端测试已经建立。桌面 E2E 可用以下命令运行：
