...
当前原创规则契约位于 [`tests/fixtures/contract-v21/scenarios`](tests/fixtures/contract-v21/scenarios)，由 `rfb-contract` 在所有平台运行；`contract-v1` 至 `contract-v20` 作为历史基准保留。
...
协议 1.21 / contract-v21 已建立内容驱动的消耗品 `UseAction`、原子单件消耗、结构化治疗结果与可观察效果触发 aware；满血无效果时只标记 tried。active baseline 共 58 个 exact fixtures。原创内容包为 1.16.0；存档继续为 v1，state hash Schema 继续为 v10。完整边界见 [Contract v21 说明](design/contract-v21-consumable-use-action.md)。

阶段 D 的种类级知识与首个消耗品纵切已经建立。下一规则切片分离实例级物品属性与知识投影；鼠标点选、路径预览和投射物动画留在后续前端切片。
...
cargo run -p rfb-contract -- validate-policy tests/fixtures/contract-v21/baseline-policy.json
```

当前 58 个原创 contract fixtures、自动协议生成、原创内容包、ASCII glyph atlas、图片 tileset manifest、缺失资源回退和 Windows Tauri 端到端测试已经建立。桌面 E2E 可用以下命令运行：
