...
物品 P3.3 在独立工作树完成探测财宝、理解、物品栏保护、启明、*启明* 和自我感知六个来源身份。金币成为返回稳定金币堆 ID 的独立探测主体；理解立即鉴定背包并提供 40 回合自动鉴定，物品栏保护在 25 回合内统一拦截当前物品损毁入口，自我感知只从实时角色投影生成报告而不增加持久字段。普通启明组合整层地图、物品和金币探测，强化启明再增加智力/智慧、整层陷阱/通路探测、背包鉴定与自我感知。demo 升至 1.202.0，正式包现有 221 种 item；审计为 196 active、106 mechanics-ready、242 blocked，content hash 为 `8c3db682991effa151df14d798fee236db536bce730cbcce23145fbf1b150772`。Rust 协议已增加 `Gold` 探测主体；协议版本、生成绑定和共享 fixtures 留待集成工作树统一收口。

物品 P3.4 在独立工作树完成黑暗、创造陷阱、照明、保护符文和毁灭五个来源身份。陷阱只替换相邻合法空地；保护符文只建立在当前无物品、金币或连接的普通地面，并按原版 550（玩家站立时为 2/3）阈值阻挡或被怪物破坏。照明与黑暗直接复用 contract-v199 已进入存档、协议投影和 State Hash 的逐格 `glow`，因此没有新增第二套楼层光照结构，也不升级协议或 Schema。毁灭先规划后一次提交 13–17 半径内的地形、普通怪物、地面物品和金币变化，保护玩家格、永久地形、入口/连接、城镇、任务楼层以及 Unique/Guardian。demo 升至 1.203.0，正式包现有 226 种 item、88 种 terrain；审计为 201 active、106 mechanics-ready、237 blocked，content hash 为 `7e31d738ec40807ac2ba1dd29727f68d3390aa03a6954b3fb4f44c097d90b04d`。共享 replay baseline 更新留待集成工作树统一收口。

物品 P3.6 在独立工作树完成远程战斗数据模型迁移：发射器保存弹药类型、原版百分制倍率、基础射程与自身加值，弹药保存类型、伤害骰、加值和损毁率；结算严格按“弹药骰与弹药伤害加值先乘发射器倍率，再加发射器伤害加值”的原版顺序执行。正式接入投石索、长弓、轻弩、重弩和 11 种普通箭、弩箭、弹丸，并同步迁移短弓/箭；竖琴和长笛保持不可射击，枪、和弓、黑箭继续由其独立 blocker 阻塞。demo 升至 1.205.0，正式包现有 246 种 item；审计为 221 active、106 mechanics-ready、217 blocked，content hash 为 `5ac6f2f84c8aed05441aa2b26940b414c4a927bf27ca60416b70e8ee8e3d1612`。协议、存档和 State Hash Schema 不变；共享 replay baseline 更新留待集成工作树统一收口。

物品 P3.7 在独立工作树正式接入经验药水、新刚特制药剂和刚特制药剂。经验药水按当前经验的一半加 10、最多 100000 点增长，并复用既有 50/100 级上限；“新刚”使用可保存的权威状态提供力量/体质各 +4 与最大生命 +50，到期统一永久损伤力量和体质；“刚”立即触发同一损伤并施加幻觉。新生与变形没有缩水实现：前者继续由角色变异与个人属性潜力系统阻塞，后者继续由角色变异候选池阻塞。demo 升至 1.206.0，正式包现有 249 种 item；审计为 224 active、106 mechanics-ready、214 blocked，content hash 为 `e32733e922438bb34b2bf67886001de280d69be532061e485f20b9fd0fb0d33f`。协议、存档结构和 State Hash Schema 不变；共享 replay baseline 更新留待集成工作树统一收口。
