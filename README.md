...
当前原创规则契约位于 [`tests/fixtures/contract-v50/scenarios`](tests/fixtures/contract-v50/scenarios)，由 `rfb-contract` 在所有平台运行；`contract-v1` 至 `contract-v49` 作为历史基准保留。
...
阶段 E 的楼层生命周期、房间内容分配、门、秘密地形、陷阱、挖掘、三层/十层地牢、最终层、持久守护者、楼层生成表、actor/loot 总预算、深度区域主题、Vault 旋转/镜像/自由落位、同层多 Vault 空间预算和第一类巢穴已经建立。Stage E 后续主要剩余 friends/escort 等动态群体、更完整的空间/陷阱预算、多入口/连通性和分支连接；任务线的暂停管理仍保留在后续队列。
...
当前 100 个原创 contract fixtures、自动协议生成、原创内容包、ASCII glyph atlas、图片 tileset manifest、缺失资源回退和 Windows Tauri 端到端测试已经建立。桌面 E2E 可用以下命令运行：
