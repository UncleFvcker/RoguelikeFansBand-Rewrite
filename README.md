...
- **道具增量**：鹤嘴锄已接入生成等级 50 起的普通掉落池，可生成既有“瓦解之”词条并使用化石为泥激活；验证范围见[当前状态](design/current-status.md)。
- **Ego 集成**：160 条身份及实例检查、旧近似清理和代表性桌面流程已落实；完整原版等价仍有六项共享生成缺口，见 [E8 审计](design/ego-integration-audit.md)。
