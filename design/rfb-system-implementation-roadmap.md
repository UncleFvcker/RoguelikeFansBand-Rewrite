...
| 射击 | 弓倍率、弹药、射程、命中、暴击、弹药破损与返回 | 部分建立 | contract-v16 与前端目标模式 v1 已建立方向/格子/实体目标、稳定选择、弹药消费、权威轨迹和首目标碰撞；下一步增加破损/回收、路径预览与动画 |
...
当前进度：contract-v16 已建立核心声明的方向/格子/实体目标规格、稳定 `TargetSelection` 与非八方向整数轨迹，前端目标模式 v1 已接入键盘准星和 `FireTarget`；内容包继续为 1.11.0。active baseline 共 53 个 exact fixtures，save v1 / state hash Schema v9 不变。下一步增加弹药破损/回收与投掷命中伤害。详细边界见 [Contract v16](contract-v16-target-selection.md)和[前端目标模式 v1](frontend-targeting-v1.md)。
...
3. **contract-v14–v16：射击、投掷与 projectile**
   ...
   - 待完成：投掷攻击 profile、破损/回收、鼠标预览和动画事件；
