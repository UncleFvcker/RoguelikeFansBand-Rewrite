...
状态：长期规则实现路线；当前基线为协议 1.8 / contract-v8
...
| 世界 tick | 每若干游戏回合处理饥饿、恢复、状态、城镇和职业回调 | 部分建立 | `worldTick` 与能量脉冲已建立；下一阶段增加明确的状态衰减、持续效果和恢复阶段 |
| RNG | 多条规则共享全局随机数 | 已建立 | 保持单一权威 RNG；为每个抽取点定义稳定顺序和测试，不按系统创建隐式 RNG |
| 事件 | 旧版规则直接打印文本和设置 redraw 标志 | 已建立基础版 | 强类型 `DomainEvent` 统一投影为协议事件；下一步让状态、抗性和效果结果进入同一事件边界 |
...
| 子系统 | 旧 RFB 行为 | 当前状态 | 新实现方案 |
| --- | --- | --- | --- |
| 玩家近战 | 命中、武器骰、攻击次数、暴击、斩味、品牌、克制、吸血等 | 部分建立 | contract-v7 已有基础命中/AC/伤害骰，contract-v8 已接入统一调度；下一步由效果管线承载 `DamagePacket` 与 `OnHitEffect` |
