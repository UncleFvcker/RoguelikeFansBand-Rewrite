...
状态：长期规则实现路线；当前基线为协议 1.18 / contract-v18
...
当前进度：contract-v18 已建立内容驱动的投掷攻击 profile、整数重量射程、独立命中/伤害检定和命中后权威落点；此前的稳定目标、弹药回收与前端目标模式继续使用。内容包为 1.13.0，active baseline 共 55 个 exact fixtures，save v1 / state hash Schema v9 不变。阶段 B 的基础伤害、抗性、效果与检定原语已足以承载普通战斗纵切。详细边界见 [Contract v18](contract-v18-thrown-attacks.md)。
...
下一步进入阶段 D：先建立背包总重量、容量限制与原子转移，再进入鉴定和掉落；程序化地牢排在物品闭环之后。
