...
状态：长期规则实现路线；当前基线为协议 1.7 / contract-v7
...
| --- | --- | --- | --- |
| 命令与行动 | 移动、等待、交互、物品、施法等命令消耗不同能量 | 部分建立 | `GameCommand → GameAction → ActionOutcome`，命令只负责意图，行动结算统一处理能量、事件和回合结束 |
| 能量与速度 | 玩家和怪物使用速度表、`energy_need` 与多次行动 | 未建立 | 整数能量调度器；固定 tie-breaker；速度只改变获得能量，不直接循环额外调用规则 |
| 世界 tick | 每若干游戏回合处理饥饿、恢复、状态、城镇和职业回调 | 未建立 | `WorldTickSchedule`，按明确阶段运行；所有阶段进入回放和 state hash |
| RNG | 多条规则共享全局随机数 | 已建立 | 保持单一权威 RNG；为每个抽取点定义稳定顺序和测试，不按系统创建隐式 RNG |
| 事件 | 旧版规则直接打印文本和设置 redraw 标志 | 部分建立 | 语义化 `GameEvent`；规则事件、UI 提示、动画提示分离；Fluent 在前端格式化 |

### 4.2 地图、视野与关卡生命周期

| 子系统 | 旧 RFB 行为 | 当前状态 | 新实现方案 |
| --- | --- | --- | --- |
| 地形格 | 墙、地板、门、楼梯、陷阱、商店入口、任务入口等 | 部分建立 | `TerrainDefinition` + `TerrainState`；静态定义与门/陷阱等运行状态分开 |
| 视野、记忆、光照 | LOS、FOV、探索记忆、怪物/物品光源 | 已建立基础版 | 保持 Rust 权威；后续增加隐身、黑暗、红外、感知和特殊视觉通道 |
| 交互地形 | 开门、关门、挖掘、撞门、解除陷阱、上/下楼 | 未建立 | 统一 `InteractTerrain` 行动和可查询交互列表，避免一个按键对应一条特殊规则路径 |
| 楼层生命周期 | 新生成、离开、持久楼层、返回、任务楼层 | 未建立 | `FloorId`、`FloorState`、`FloorRepository`；存档保存访问过的持久楼层，临时楼层按种子重建或保存快照 |
| 地图生成 | 房间、走廊、vault、巢穴、主题、守护者、物品与怪物分配 | 未建立 | 多阶段生成管线：布局 → 地形 → 连通性 → 房间主题 → 实体 → 出入口 → 校验；每阶段使用独立命名 RNG 域但由主 RNG 派生确定性种子 |
...
| 派生属性 | AC、速度、命中、伤害、攻击次数、抗性、感知等多来源叠加 | 部分建立 | `DerivedStatsPipeline`：基础 → 种族 → 职业 → 性格 → 装备 → 状态 → 姿态 → 环境；输出带来源的诊断结果 |
...
| 子系统 | 旧 RFB 行为 | 当前状态 | 新实现方案 |
| --- | --- | --- | --- |
| 基础状态 | 加速、减速、失明、混乱、恐惧、中毒、流血、眩晕、麻痹等 | 未建立 | `StatusInstance { kindId, intensity, remaining, source }`；叠加、替换和衰减由定义声明 |
| 抗性 | 基础元素、高级元素、免疫、弱点、临时抗性 | 未建立 | 抗性等级枚举而非大量 bool；伤害类型统一走 `ResistanceResolver` |
...
| 子系统 | 旧 RFB 行为 | 当前状态 | 新实现方案 |
| --- | --- | --- | --- |
| 玩家近战 | 命中、武器骰、攻击次数、暴击、斩味、品牌、克制、吸血等 | 部分建立 | contract-v7 已有基础命中/AC/伤害骰；下一步拆成 `AttackProfile`、`HitResolver`、`DamagePacket`、`OnHitEffect` |
...
| 伤害与死亡 | 多种伤害类型、减伤、死亡原因、怪物击杀和掉落 | 部分建立 | `DamagePacket` 记录来源、类型、标签和阶段；死亡生成单一 `DeathOutcome`，后续系统监听击杀、任务、经验和掉落 |
...
| 目标选择 | 自身、方向、格子、怪物、范围、投射、锥形等 | 未建立 | `TargetSpec` 由核心声明，前端进入目标模式并提交稳定 `TargetSelection` |
| 效果执行 | 伤害、治疗、传送、召唤、侦测、地形改变、附魔等 | 未建立 | 建立声明式 `EffectSpec` 组合器；少量无法组合的规则通过受控 Rust handler 注册 |
...
| 子系统 | 旧 RFB 行为 | 当前状态 | 新实现方案 |
| --- | --- | --- | --- |
| 背包与地面堆 | 堆叠、拆分、容量、溢出、拾取、丢弃 | 已建立基础版 | 扩展重量、容量、原子转移和容器；保持实例 ID 不复用 |
...
| 鉴定与感知 | aware、tried、伪鉴定、已知 flag、诅咒发现 | 未建立 | 真实物品实例与 `ItemKnowledge` 分离；UI 永远读取可见视图，不能直接看到隐藏属性 |
...
| 回合与移动 AI | 追踪、视线、气味/flow、保持距离、逃跑、守卫、射击 | 未建立 | `AiIntent` 只选择行动意图；寻路、目标评估和规则结算分开；确定性 tie-breaker |
| 怪物施法 | 选择法术、频率、射线检查、召唤和智能学习 | 未建立 | 能力系统共用 effect；AI 仅从可用能力评分，不直接执行效果 |
| 群体与生成 | 成群、护卫、朋友、召唤、繁殖、独特和守护者 | 未建立 | `SpawnGroupDefinition` 与 `SpawnReason`；唯一性和种群上限属于世界状态 |
| 怪物物品与掉落 | 携带物、偷窃、掉落次数和主题 | 未建立 | 怪物 inventory 可选组件；死亡统一生成 loot outcome |
...
1. **contract-v8：行动能量与怪物追踪**
   - 拆分核心模块；
   - actor speed、energy、行动队列；
   - 基础寻路和追踪；
   - 多怪物稳定顺序；
   - 不增加大批内容。
2. **contract-v9：状态/效果/抗性基础**
   - 通用 effect executor；
   - 伤害类型和抗性等级；
   - 毒、眩晕、恐惧、加速、减速；
   - 状态 UI 和存档/回放。
3. **contract-v10：武器与多 blow 战斗**
   - 武器实例伤害和命中；
   - 玩家攻击次数；
   - 怪物 blow 列表；
   - `DamagePacket`、死亡/掉落接口；
   - 为后续射击和法术保留 projectile/effect 边界。

完成这三步后，再进入物品鉴定和程序化地牢，收益高于现在直接制作职业或法术内容。
