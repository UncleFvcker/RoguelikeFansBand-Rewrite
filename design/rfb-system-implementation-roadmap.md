...
状态：长期规则实现路线；当前基线为协议 1.59 / contract-v59
...
当前进度：阶段 B 的基础伤害、抗性、效果与检定原语已足以承载普通战斗纵切；contract-v21 已复用 `EffectSpec::Heal` 完成首个消耗品，contract-v29/v31 又复用结构化 check 完成门与搜索检定。当前 active baseline 已进入阶段 E 的 contract-v59，阶段 B 后续只按实际规则入口补充新的状态、抗性和效果。
