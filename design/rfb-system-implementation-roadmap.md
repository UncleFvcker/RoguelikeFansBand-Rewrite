...
状态：长期规则实现路线；当前基线为协议 1.89 / contract-v89
...
当前阶段定位：截至 contract-v89，重构已经形成确定性、可存档、可回放并可持续扩展的规则引擎，阶段 E 的地牢生成与探索闭环达到阶段性里程碑，阶段 F 已建立原版式角色成长、构筑和首轮技能检定，阶段 G 已完成玩家 Mana、能力书、目标施法及伤害/治疗/位移/召唤/侦测/地形/状态效果的首轮纵切；阶段 H 已建立怪物百分比施法、效用权重、阵营目标、多格结算、敌对召唤、保持距离/受伤撤退、smart 抗性观察，以及玩家召唤物的行动/命令闭环。项目尚未达到旧 RFB 的完整职业矩阵、法术广度、怪物生态、世界经济或内容规模。
...
当前进度：阶段 B 的基础伤害、抗性、效果与检定原语已承载普通战斗与玩家施法纵切；contract-v73–v85 逐步固定伤害、治疗、范围、射线、锥形、位移、召唤、侦测、地形、状态和有序效果。contract-v86–v88 让怪物 caster 复用直接/自身/多格伤害、状态、抗性、死亡、召唤和逐目标结果，并只从实际玩家结算学习有限抗性；contract-v89 让玩家召唤物复用 actor 移动、近战和玩家击杀归属。当前 active baseline 已进入 contract-v89，后续只按实际规则入口补充新的状态、抗性和效果。
...
当前进度：contract-v26–v69 已建立从程序化楼层到树状多 dungeon、Vault、区域主题、群体、入口守卫、campaign 与实例生命周期的地牢纵切；普通 Echo/Resonance 仍回地表即清理，Archive 覆盖 retained/TTL。运行时地形破坏直接写入权威地图，不触发自动连通修复。角色成长与技能见 contract-v70–v72，玩家施法循环见 contract-v73–v85，怪物施法、阵营目标、战术移动与有限记忆见 contract-v86–v88，友方召唤物行动/命令见 contract-v89。当前内容包为 1.80.0，content hash 为 `29116f924e1ef4ddf6b0aa43f3b1b1bd0b4d28245ac086bce30d7a008e8e9e8e`，active baseline 共 272 个 exact fixtures，save v1 / state hash Schema v39。详细边界见 [Contract v89](contract-v89-friendly-summon-commands.md)。
...
## 8. contract-v75–v89 阶段性里程碑

### 8.1 基线与完成度判断

当前权威基线为协议 1.89、内容包 1.80.0、contract-v89、save v1 和 state hash Schema v39；内容 hash 为 `29116f924e1ef4ddf6b0aa43f3b1b1bd0b4d28245ac086bce30d7a008e8e9e8e`。active baseline 包含 272 个 exact fixtures，零 waiver。v73–v85 已建立最小玩家施法循环及伤害、治疗、范围、射线、锥形、位移、召唤、侦测、地形、状态和有序效果；v86 在同一效果管线上增加 Monster 百分比施法频率、声明顺序权重、射程/墙体/友军 clean-shot 过滤、频率失败回退和按自身行动计数的逆频率冷却；v87 增加 HP/状态/距离有效权重、自身/多格目标、保守 footprint 风险和敌对召唤；v88 增加阵营目标、敌我计数、多目标结算、战术移动和 smart 抗性观察；v89 增加友方召唤物行动、全局命令、玩家击杀归属和附近跨层跟随。Race/Class/Personality、技能成长、出生装备、自然属性、HP 序列、胜利后等级 100 / `18/820` 和装备派生边界保持一致。
...
P29“友方召唤物行动与首版命令”已由 contract-v89 完成。下一执行里程碑建议为 P30“首个非 Mana 职业资源”：

1. 从原版职业资源中选择一个代表，明确获得、上限、衰减/恢复和可消费时点；
2. 复用现有 `ResourcePool`/ability 管线，但避免把 Mana 的等待/休息恢复假定扩散到所有资源；
3. 给至少一个既有或新增构筑配置该资源与两种消费能力，证明内容声明而非职业硬编码；
4. 固定 UI、save/replay、旧存档默认值、拒绝边界和 exact fixtures。

阶段 H 已由 P26–P29 建立怪物施法、阵营目标、战术移动、有限记忆与玩家召唤物闭环；P30 回到阶段 G/F 的职业差异。地牢系统继续只修复阻塞角色/法术验证的缺陷。
