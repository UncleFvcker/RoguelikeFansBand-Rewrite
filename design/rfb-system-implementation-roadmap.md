...
状态：长期规则实现路线；当前基线为协议 1.93 / contract-v93（P31–P36 进展见 8.3 与[待实现内容清单](pending-implementation.md)）
...
**实际走向修订（2026-07）**：用户确认多职业资源"有通用接口即可"，后续内容改由旧版导入管线供给。P31 建立 f_info/r_info 只读导入管线（`.local/` 本地包 + 缺口报告），P32–P36 按缺口报告覆盖数依次落地规则族并回灌导入映射：多 blow 近战（P32）、状态/治疗法术（P33）、位移族（P34，contract-v91）、混乱/致盲/麻痹状态族（P35，contract-v92）、bolt/ball 直伤族与伤害平坦加值（P36，contract-v93）。DETECT 侦测族经源码核实为 `MST_POSSESSOR` 附身专用组（怪物不施放），按不适用归档。下一执行里程碑为 **P37「BR_ 吐息族」**（790 实例，伤害 = 当前 HP 百分比封顶，需新 breath-damage 效果形态），其后按报告排期伤害类型扩展、召唤族与小型效果形态；滚动缺口清单见[待实现内容清单](pending-implementation.md)的 contract-v93 遗留段。
