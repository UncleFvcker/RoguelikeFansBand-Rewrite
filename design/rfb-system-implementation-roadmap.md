...
状态：长期规则实现路线；当前基线为协议 1.10 / contract-v10
...
| 事件 | 旧版规则直接打印文本和设置 redraw 标志 | 已建立基础版 | 强类型 `DomainEvent` 已覆盖战斗、物品和状态生命周期并统一投影为协议事件；contract-v11 补充伤害类型、抗性结果和检定上下文 |
...
当前进度：contract-v10 已在 v9 底座上增加流血和内容驱动元素近战。周期状态按稳定 kind ID 顺序执行；角色内容声明伤害类型，物理攻击走 AC，元素攻击走对应抗性。原创余烬微光造成火焰伤害，协议 1.10 输出近战伤害类型，内容包升级到 1.6.0。active baseline 共 39 个 exact fixtures。下一步为其他基础元素增加实际入口，并在属性/检定管线完成后实现眩晕和恐惧。详细边界见 [Contract v10](contract-v10-bleeding-elemental-melee.md)。
...
1. **contract-v11：阶段 B 收尾与检定接口**
   - 伤害类型、抗性结果和检定上下文进入强类型领域事件；
...
3. **contract-v13：射击、投掷与 projectile**
   ...
   - 物品实例位置变化、破损/回收和动画事件；
   - 与既有 `DamagePacket` / effect pipeline 共用结算。
