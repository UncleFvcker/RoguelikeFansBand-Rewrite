...
状态：长期规则实现路线；当前基线为协议 1.17 / contract-v17
...
当前进度：contract-v17 已建立内容驱动的弹药破损、无实体碰撞时的无 RNG 回收，以及权威地面实例事务；此前的稳定目标与前端目标模式继续使用。内容包为 1.12.0，active baseline 共 54 个 exact fixtures，save v1 / state hash Schema v9 不变。下一步建立投掷攻击 profile、重量射程和命中/伤害。详细边界见 [Contract v17](contract-v17-ammunition-recovery.md)。
...
3. **contract-v14–v17：射击、投掷与 projectile**
   ...
   - 待完成：投掷攻击 profile、重量射程、鼠标预览和动画事件；
