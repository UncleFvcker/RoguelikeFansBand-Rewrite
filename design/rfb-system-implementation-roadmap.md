...
状态：长期规则实现路线；当前基线为协议 1.15 / contract-v15
...
| 射击 | 弓倍率、弹药、射程、命中、暴击、弹药破损与返回 | 部分建立 | contract-v15 已建立方向射击、发射器 profile、稳定弹药引用/消费、权威轨迹和首目标碰撞；下一步增加目标选择和破损/回收 |
...
当前进度：contract-v15 已建立稳定弹药引用/消费、碰撞点与落点分离、单件投掷实例事务；原创内容包 1.11.0 新增共鸣弹丸。active baseline 共 52 个 exact fixtures，save v1 / state hash Schema v9 不变。下一步建立目标选择协议，并增加弹药破损/回收与投掷命中伤害。详细边界见 [Contract v15](contract-v15-ammunition-throwing.md)。
...
3. **contract-v14–v15：射击、投掷与 projectile**
   - 已完成：方向、射程、轨迹、碰撞点和可放置落点；
   - 已完成：发射器 profile、稳定弹药引用/消费与投掷实例拆分；
   - 待完成：目标选择、投掷攻击 profile、破损/回收和动画事件；
