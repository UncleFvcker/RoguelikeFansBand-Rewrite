...
状态：长期规则实现路线；当前基线为协议 1.16 / contract-v16
...
| 射击 | 弓倍率、弹药、射程、命中、暴击、弹药破损与返回 | 部分建立 | contract-v16 已建立方向/格子/实体目标、发射器 profile、稳定弹药消费、权威轨迹和首目标碰撞；下一步增加前端目标模式与破损/回收 |
| 投掷 | 物品重量、投掷技能、返回武器和药水破裂 | 部分建立 | contract-v15 已建立单件实例拆分、权威轨迹与安全落点；下一步增加重量射程、命中/伤害和破裂 |
...
| 目标选择 | 自身、方向、格子、怪物、范围、投射、锥形等 | 部分建立 | contract-v16 已由核心声明方向/格子/实体 `TargetSpec` 并接收稳定 `TargetSelection`；下一步接入前端目标模式，再扩展自身、范围和锥形 |
...
当前进度：contract-v16 已建立核心声明的方向/格子/实体目标规格、稳定 `TargetSelection` 与非八方向整数轨迹；内容包继续为 1.11.0。active baseline 共 53 个 exact fixtures，save v1 / state hash Schema v9 不变。下一步接入前端目标模式，再增加弹药破损/回收与投掷命中伤害。详细边界见 [Contract v16](contract-v16-target-selection.md)。
...
3. **contract-v14–v16：射击、投掷与 projectile**
   - 已完成：方向、射程、轨迹、碰撞点和可放置落点；
   - 已完成：发射器 profile、稳定弹药引用/消费与投掷实例拆分；
   - 已完成：核心方向/格子/实体目标规格、稳定选择与非八方向整数轨迹；
   - 待完成：前端目标模式、投掷攻击 profile、破损/回收和动画事件；
   - 射击已与既有 `DamagePacket` / effect pipeline 共用结算，投掷伤害后续接入。

完成这三步后，再进入物品鉴定和程序化地牢，收益高于现在直接制作职业或大批法术内容。
