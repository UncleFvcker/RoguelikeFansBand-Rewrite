...
状态：长期规则实现路线；当前基线为协议 1.88 / contract-v88
...
当前阶段定位：截至 contract-v88，重构已经形成确定性、可存档、可回放并可持续扩展的规则引擎，阶段 E 的地牢生成与探索闭环达到阶段性里程碑，阶段 F 已建立原版式角色成长、构筑和首轮技能检定，阶段 G 已完成玩家 Mana、能力书、目标施法及伤害/治疗/位移/召唤/侦测/地形/状态效果的首轮纵切；阶段 H 已建立怪物百分比施法、效用权重、阵营目标、多格结算、敌对召唤、保持距离/受伤撤退和 smart 抗性观察。项目尚未达到旧 RFB 的完整职业矩阵、法术广度、怪物生态、世界经济或内容规模。
...
| 回合与移动 AI | 追踪、视线、气味/flow、保持距离、逃跑、守卫、射击 | 部分建立 | contract-v8 已有八方向 BFS 追踪、占位避让和确定性 tie-breaker；contract-v88 加入内容阈值驱动的保持距离与受伤撤退。后续扩展安全路径、气味/flow、守卫和射击 |
...
当前进度：阶段 B 的基础伤害、抗性、效果与检定原语已承载普通战斗与玩家施法纵切；contract-v73–v85 逐步固定伤害、治疗、范围、射线、锥形、位移、召唤、侦测、地形、状态和有序效果。contract-v86–v88 让怪物 caster 复用直接/自身/多格伤害、状态、抗性、死亡、召唤和逐目标结果，并只从实际玩家结算学习有限抗性。当前 active baseline 已进入 contract-v88，后续只按实际规则入口补充新的状态、抗性和效果。
...
当前进度：contract-v26–v69 已建立从程序化楼层到树状多 dungeon、Vault、区域主题、群体、入口守卫、campaign 与实例生命周期的地牢纵切；普通 Echo/Resonance 仍回地表即清理，Archive 覆盖 retained/TTL。运行时地形破坏直接写入权威地图，不触发自动连通修复。角色成长与技能见 contract-v70–v72，玩家施法循环见 contract-v73–v85，怪物施法、阵营目标、战术移动与有限记忆见 contract-v86–v88。当前内容包为 1.80.0，content hash 为 `29116f924e1ef4ddf6b0aa43f3b1b1bd0b4d28245ac086bce30d7a008e8e9e8e`，active baseline 共 265 个 exact fixtures，save v1 / state hash Schema v38。详细边界见 [Contract v88](contract-v88-monster-targets-tactics-memory.md)。
...
## 8. contract-v75–v88 阶段性里程碑

### 8.1 基线与完成度判断

当前权威基线为协议 1.88、内容包 1.80.0、contract-v88、save v1 和 state hash Schema v38；内容 hash 为 `29116f924e1ef4ddf6b0aa43f3b1b1bd0b4d28245ac086bce30d7a008e8e9e8e`。active baseline 包含 265 个 exact fixtures，零 waiver。v73–v85 已建立最小玩家施法循环及伤害、治疗、范围、射线、锥形、位移、召唤、侦测、地形、状态和有序效果；v86 在同一效果管线上增加 Monster 百分比施法频率、声明顺序权重、射程/墙体/友军 clean-shot 过滤、频率失败回退和按自身行动计数的逆频率冷却；v87 增加 HP/状态/距离有效权重、自身/多格目标、保守 footprint 风险和敌对召唤；v88 增加阵营目标、敌我计数、多目标结算、战术移动和 smart 抗性观察。Race/Class/Personality、技能成长、出生装备、自然属性、HP 序列、胜利后等级 100 / `18/820` 和装备派生边界保持一致。
...
下一执行里程碑建议为 P29“友方召唤物行动与首版命令”：

1. 让玩家拥有的召唤物复用 v88 阵营目标集合，按稳定距离选择敌对怪物并执行首版移动/近战；
2. 增加跟随、攻击、保持距离和守卫四种内容/命令模式，固定切换的行动成本与 RNG 边界；
3. 明确主人死亡、召唤到期、跨楼层、楼层清理和无合法目标时的回退；
4. 为 save/replay、协议事件和 contract fixtures 建立稳定命令与行动结果。

阶段 H 已由 P26–P28 建立怪物施法、阵营目标、战术移动和有限记忆层；P29 优先闭合已存在但仍静止的玩家召唤物。多资源职业可在该行动边界之后恢复排期；地牢系统继续只修复阻塞角色/法术验证的缺陷。
