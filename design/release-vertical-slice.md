...
The next user-selected product expansion is the Outpost supply loop. Its Gate 0 contract preserves `demo.floor.surface` as the stable return floor and orders the work as gold source -> hunger consumer -> fuel lighting -> Outpost content -> General Store transactions -> UI acceptance. Gates 1-5 are complete at contract-v161: Warrior birth gold/food/torches, Warrens gold and supply attempts, wallet pickup, hunger, natural recovery, instance fuel, refueling, dungeon darkness, authoritative Outpost identity, and General Store inventory/pricing/trading are covered by 461 exact fixtures with no waiver. Gate 6, the player-facing shop UI and full acceptance flow, is next. Phase 17's remaining release acceptance is not silently treated as proof of these new systems.
