...
Historical scope: this document preserves the Phase 17 journey decisions and gate results. Its Warrior-only entry, version numbers and completion claims describe that phase, not today's available builds or release acceptance. For the current four-part status and the 2026-09-09 acceptance scope, see [Current status](current-status.md).

Status: Phase 17 is active. Gates 0-4 are complete; Gate 5 is active. Gate 0 fixed the original journey contract against base commit `09291410`. After Gate 2, the user replaced the original Echo target with RFB's Warrens as the first player flow and explicitly allowed Echo Depths to stay out of the release journey. Gate 3 completed that bounded content and campaign migration. The post-Gate-3 contract-v151 amendment then made RFB Warrior the sole production career, adopted RFB's Standard humanoid body for that build, and replaced the staged objective strip with factual dungeon/depth/Boss status. Gate 4 completed the result, restart, and native-save recovery presentation. Gate-5 corrections v153–v156 replace the fallback dungeon maps and placeholder surface presentation with seeded Warrens maps, an explicit outdoor entrance, state-aware connection controls, free room placement, mineral veins, RFB-scale initial monster density, and source-aligned floor/monster loot. Contextual onboarding remains. The user-directed successor track is [Phase 18: Outpost supply loop](phase-18-outpost-supply-loop.md); Gates 1-6 and post-Gate expansion now cover gold, hunger, fuel/light, a walled Outpost, eight transactional shops, a persistent Home, and their UI through contract-v167 without retroactively widening Phase 17. The current baseline is protocol `1.134`, save v1, state-hash Schema `61`, content `1.157.0`, contract-v167 and 467 exact fixtures.
...
Gate 5 begins from contract-v153. The Warrens double-rectangle fallback was corrected to a fixed-source-informed but independently authored 66×22 generator: five mostly irregular connected rooms, shuffled ring corridors, and the original shallow-dungeon stair density. Contract-v156 now also provides area-scaled room/anywhere items, source-aligned monster drop counts, remains, and the final Speed reward; a 16-seed nine-floor matrix fixes connectivity, variation, item depth, same-seed determinism, and stored-floor persistence. Production session construction still selects `demo.world.warrens-journey` and `demo.build.warrior`; the world owns the one-dungeon victory requirement, while the race owns the Standard body. The user has already completed a manual run, so the next useful work is to close concrete usability or content gaps found during normal play rather than repeat a synthetic completion matrix solely for balance certification. The original lab and old demo builds remain regression content rather than player-facing choices.

The next user-selected product expansion is the Outpost supply loop. Its Gate 0 contract preserves `demo.floor.surface` as the stable return floor and orders the work as gold source -> hunger consumer -> fuel lighting -> Outpost content -> General Store transactions -> UI acceptance. Gates 1-6 are complete. Contract-v162 added Temple and Alchemist; contract-v163 replaced the temporary supply-court layout with a walled town, placed Warrens outside the east gate, and added an RFB-sourced Magic Shop beside the Alchemist; contract-v164 added a shared Armoury/Weaponsmith forge; contract-v165 added the original town-stock Bookstore; contract-v166 added the original Home as a separate persistent storage facility; contract-v167 adds the original Black Market with its Warrior markup and non-town high-level books. The 467 exact fixtures and desktop supply-loop E2E cover the player-facing transaction, storage, and save flow without waiver. Phase 17's remaining release acceptance is not silently treated as proof of these new systems.
