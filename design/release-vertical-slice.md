...
The next user-selected product expansion is the Outpost supply loop. Its Gate 0 contract preserves `demo.floor.surface` as the stable return floor and orders the work as gold source -> hunger consumer -> fuel lighting -> Outpost content -> General Store transactions -> UI acceptance. Gates 1-6 are complete, and contract-v162 adds the first post-Gate expansion: General Store, Temple, and Alchemist share a three-entrance supply court while retaining independent persistent state. The 462 exact fixtures and desktop supply-loop E2E cover the full player-facing transaction and save flow without waiver. Phase 17's remaining release acceptance is not silently treated as proof of these new systems.
