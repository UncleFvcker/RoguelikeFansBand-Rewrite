...
The next user-selected product expansion is the Outpost supply loop. Its Gate 0 contract preserves `demo.floor.surface` as the stable return floor and orders the work as gold source -> hunger consumer -> fuel lighting -> Outpost content -> General Store transactions -> UI acceptance. Gates 1-6 are complete. Contract-v162 added Temple and Alchemist; contract-v163 replaced the temporary supply-court layout with a walled town, placed Warrens outside the east gate, and added an RFB-sourced Magic Shop beside the Alchemist; contract-v164 added a shared Armoury/Weaponsmith forge; contract-v165 adds the original town-stock Bookstore to the shared arcane building. The 465 exact fixtures and desktop supply-loop E2E cover the full player-facing transaction and save flow without waiver. Phase 17's remaining release acceptance is not silently treated as proof of these new systems.
