...
- Outpost 出生层不再执行荒野怪物分配；普通局部荒野按原版道路 4 次、非道路 10 次分配，伏击仍独立执行 20 次分配。
...
contract-v221 接入冰冻球、跳跃火球和球状闪电，严格同步增至 300 条，actor
增至 365，ability 保持 152。接触光环只接受毒素、火焰、冰冷和闪电；三个新增
类型按抗性即时扣血，毒素仍转为持续状态。导入器按原版等级公式生成三条 `1d2`
光环，既有 `8d8` 同元素死亡爆炸继续复用。十七级怪物完成收口。

contract-v222 接入 33 只跨 18–20 级的无怪物施法怪物，严格同步增至 333 条，
actor 增至 398，ability 保持 152。该批只复用既有接触光环、繁殖、死亡爆炸、
碎岩易伤、毁墙/换位、骑乘、水生与掉落结构；参数化施法与四个机制阻塞未混入。

contract-v223 接入 15 只只使用现有能力的 P37B 怪物，严格同步增至 348 条，
actor 增至 413，ability 保持 152。六种吐息、闪现/拉近、尖叫和状态诅咒
全部共享既有 ability；附身者侦测令牌未进入 casting profile。

contract-v224 接入 11 只 P38A 参数化伤害怪物，严格同步增至 359 条，actor
增至 424，ability 增至 163。新增内容只为既有 bolt、ball 和 breath effect
保存权威参数；没有新增运行时 effect、通用框架或兼容路径。

contract-v225 接入 6 只 P38B 治疗与召唤参数怪物，严格同步增至 365 条，
actor 增至 430，ability 增至 171。8 条内容记录只保存既有治疗、分类召唤、
同族召唤和伤害 effect 参数；没有新增运行时 effect 或通用框架。

contract-v226 接入闪烁的灯光与黏糊恶心女王，严格同步增至 367 条，actor
增至 432，ability 增至 174。`JMP_LIGHT(5d5)` 使用窄 `jump-damage` 固定先范围
光伤、后闪现；接触光环迁移为有序列表，女王的毒素、酸性 `2d3` 逐项复用现有
结算和死亡中止，不建立通用 effect 序列框架。

contract-v228 接入变色龙，严格同步增至 368 条，actor 增至 433，ability 保持
174。新 `chameleon` 标签不复用 P20 的纯外观 `shapechanger` 近似：本体身份
固定，当前形态以既有 `appearanceKindId` 保存，每次清醒行动以 `1/13` 换形；
形态直接驱动生命、速度、抗性、属性、近战、移动和施法。没有新增通用状态、
effect、协议字段或兼容层。

contract-v229 接入妖鬼，严格同步增至 369 条，actor 增至 434，ability 保持
174。`eldritch-horror` 只在敌对怪物新进入视野时运行；等级、Unique/群体修正、
首次与低概率重触发、玩家豁免共同决定是否进入后果链。混乱、幻觉、麻痹、属性
损伤、当前层地图失忆和既有精神变异均复用现有消费者，只新增实例触发标记。
