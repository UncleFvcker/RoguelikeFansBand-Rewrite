...
| T4 | P53–P61（Death 四册 + 设备/恢复消耗品 ✅） | **玩家领域法术逐册映射**（Death 四册已完成）∥ **设备/消耗品效果系统**（P58–P61 见 [contract-v108](contract-v108-charged-items.md)、[contract-v109](contract-v109-dynamic-devices.md)、[contract-v110](contract-v110-device-recharge.md) 与 [contract-v111](contract-v111-restorative-items.md)） | 32 个能力、4 本书和 384 行职业覆盖保持；实例充能、动态 profile、首批 wand/staff/rod、恢复/主动充能及有序状态/资源恢复已建立，`consumable-effect` 95→81。`device-effect` 61 经核对均为卷轴，P62 先重分类并盘点卷轴事务 |
| T5 | 后续 | **d_info 地牢 + v_info vault 导入** → q_* 任务 → 城镇/荒野 | 世界内容线；荒野/城镇需先补系统设计 |

## 5. 滚动修订

每轮导入后按缺口报告数字重排 T4+ 顺序。P53–P57 已完成 Death 四册：12 个静态职业映射 32 个法术、384 行参数，Death 效果缺口 480→96；Rogue（敏捷）、Blood Mage（生命）和 Skillmaster（动态）保留未接。P58 建立实例充能/治疗骰并接入六种固定治疗药水；P59 物化动态设备 profile、power、目标与随机容量；P60 建立差异化自然恢复、持久余数和主动充能；P61 建立有序状态/资源恢复并接入八种新恢复消耗品映射，使 `consumable-effect` 95→81。`device-effect` 61 经审计全部是 tval 70/71 卷轴而非剩余设备，P62 优先盘点知识、传送、侦测与附魔事务；`artifact-activation` 180、`ego-activation` 13、法术清尾及 Invoke Spirits 四项通用系统缺口继续保留。本文件随每次重排更新。
