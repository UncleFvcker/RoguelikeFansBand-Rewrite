...
| 梯队 | 迭代 | 内容 | 理由 |
| --- | --- | --- | --- |
| T1 | P48 | **b_info + 种族 + 性格导入**（纯导入器）：113 模板挂 75 种族（stats/life/exp/skills/body 可表达面全导，旗标/powers 入缺口报告）、~20 性格全导 | P47 表面刚就绪；体量小；.local 包获得角色定义自洽性；缺口报告首次量化种族旗标覆盖数 |
| T2 | P49 | **装备/内在旗标系统**（引擎）+ 回灌重跑导入 | 一次解锁词条 72 条 inexpressible、神器旗标主体、种族内在旗标；此时三处覆盖数据最全 |
