...
| T4 | P53+（Death 第一册 ✅） | **玩家领域法术逐册映射**（P53/P54 已完成 Death 第一册，见 [legacy-player-spell-import-v1](legacy-player-spell-import-v1.md) 与 [contract-v104](contract-v104-death-first-book.md)）∥ **设备/消耗品效果系统**（可并行插队） | `abilityOverrides` 已解决职业参数差异；等级缩放、actor Detect、sleep、临时抗性和 Control 已成为复用词汇。P55 优先按系统边界推进 Death 第二册 |
| T5 | 后续 | **d_info 地牢 + v_info vault 导入** → q_* 任务 → 城镇/荒野 | 世界内容线；荒野/城镇需先补系统设计 |

## 5. 滚动修订

每轮导入后按缺口报告数字重排 T4+ 顺序。P53/P54 已完成 Death 第一册：12 个静态职业映射 8 个法术、96 行参数，Death 效果缺口 480→384；Rogue（敏捷）、Blood Mage（生命）和 Skillmaster（动态）保留未接。P55 优先盘点并推进 Death 第二册：现有缩放、area/bolt/beam 可复用，但活体限定、bolt-or-beam、自身中心 AoE、灭绝、武器品牌、吸血和尸体实体必须分别建模。法术清尾（S_ 字形召唤 177/SHRIEK/TRAPS）与设备效果系统继续可插队。本文件随每次重排更新。
