...
| T4 | P53–P67（Death 四册 + 设备/恢复消耗品 + 鉴定/侦测/传送/附魔/诅咒/召唤卷轴 ✅） | **玩家领域法术逐册映射**（Death 四册已完成）∥ **设备/消耗品效果系统**（P58–P67 见 contract-v108–v117） | 32 个能力、4 本书和 384 行职业覆盖保持；实例充能、动态 profile、恢复/主动充能、有序恢复、鉴定、地图/侦测、传送/召回、附魔、诅咒及四种召唤卷轴已建立。`consumable-effect` 95→81；`scroll-effect` 61→34，`device-effect` 退出报告 |
| T5 | 后续 | **d_info 地牢 + v_info vault 导入** → q_* 任务 → 城镇/荒野 | 世界内容线；荒野/城镇需先补系统设计 |

## 5. 滚动修订

每轮导入后按缺口报告数字重排 T4+ 顺序。P53–P57 已完成 Death 四册：12 个静态职业映射 32 个法术、384 行参数，Death 效果缺口 480→96；Rogue（敏捷）、Blood Mage（生命）和 Skillmaster（动态）保留未接。P58 建立实例充能/治疗骰并接入六种固定治疗药水；P59 物化动态设备 profile、power、目标与随机容量；P60 建立差异化自然恢复、持久余数和主动充能；P61 建立有序状态/资源恢复并接入八种新恢复消耗品映射，使 `consumable-effect` 95→81；P62 把 tval 70/71 独立为 `scroll-effect` 并接入普通/完全鉴定；P63 接入地图/侦测七条；P64 接入五种传送/回城事务；P65 接入五种装备附魔事务；P66 接入四种施咒/解除事务；P67 接入四种怪物/亡灵/宠物/同族召唤，使卷轴缺口 61→59→52→47→42→38→34。下一轮先重排剩余世界/状态/物品卷轴；`artifact-activation` 180、`ego-activation` 13、法术清尾及 Invoke Spirits 四项通用系统缺口继续保留。本文件随每次重排更新。
