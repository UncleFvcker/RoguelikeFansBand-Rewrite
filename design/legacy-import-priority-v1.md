...
| T4 | P53+（Death 第一至三册 ✅） | **玩家领域法术逐册映射**（P53–P56 已完成 Death 前三册，见 [legacy-player-spell-import-v1](legacy-player-spell-import-v1.md)、[contract-v105](contract-v105-death-second-book.md) 与 [contract-v106](contract-v106-death-third-book.md)）∥ **设备/消耗品效果系统**（可并行插队） | `abilityOverrides`、等级缩放、职业 beam 档案、灭绝、尸体/复活、随机效果、永久武器 affix、吸血与全可见目标伤害已成为复用词汇。P57 候选为 Death 第四册逐槽盘点；设备纵切仍可按覆盖收益插队 |
| T5 | 后续 | **d_info 地牢 + v_info vault 导入** → q_* 任务 → 城镇/荒野 | 世界内容线；荒野/城镇需先补系统设计 |

## 5. 滚动修订

每轮导入后按缺口报告数字重排 T4+ 顺序。P53–P56 已完成 Death 前三册：12 个静态职业映射 24 个法术、288 行参数，Death 效果缺口 480→192；Rogue（敏捷）、Blood Mage（生命）和 Skillmaster（动态）保留未接。P56 建模了随机状态时长、状态派生加值、RandomChoice/NoOp、敌对固定召唤、永久武器 affix、Vampiric 近战吸血、重复追踪 Drain Life、全可见目标伤害与 prorated 曲线。P57 候选为 Death 第四册逐槽盘点；法术清尾（S_ 字形召唤 177/SHRIEK/TRAPS）、Invoke Spirits 四项通用系统缺口与设备效果系统继续可按真实覆盖收益插队。本文件随每次重排更新。
