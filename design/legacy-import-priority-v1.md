...
| T4 | P53–P60（Death 四册 + 充能/动态设备 ✅） | **玩家领域法术逐册映射**（Death 四册已完成）∥ **设备/消耗品效果系统**（P58–P60 见 [contract-v108](contract-v108-charged-items.md)、[contract-v109](contract-v109-dynamic-devices.md) 与 [contract-v110](contract-v110-device-recharge.md)） | 32 个能力、4 本书和 384 行职业覆盖保持；实例充能、动态 profile、首批 wand/staff/rod、差异化自然恢复和资源/设备主动充能已建立，`consumable-effect` 95→89，`device-effect` 64→61。P61 按真实覆盖收益选择后续设备、artifact/ego activation 或消耗品效果族 |
| T5 | 后续 | **d_info 地牢 + v_info vault 导入** → q_* 任务 → 城镇/荒野 | 世界内容线；荒野/城镇需先补系统设计 |

## 5. 滚动修订

每轮导入后按缺口报告数字重排 T4+ 顺序。P53–P57 已完成 Death 四册：12 个静态职业映射 32 个法术、384 行参数，Death 效果缺口 480→96；Rogue（敏捷）、Blood Mage（生命）和 Skillmaster（动态）保留未接。P58 建立实例充能/治疗骰并接入六种固定治疗药水，`consumable-effect` 95→89；P59 进一步物化动态设备 profile、power、目标与随机容量，为原版三种通用壳生成首批候选，`device-effect` 64→61；P60 建立 rod 与 wand/staff 的差异化自然恢复、持久余数，以及职业资源/设备来源主动充能和失败损毁边界。P61 按 `artifact-activation` 180、`ego-activation` 13、`device-effect` 61 和 `consumable-effect` 89 的实际覆盖收益选择下一纵切；法术清尾及 Invoke Spirits 四项通用系统缺口继续保留。本文件随每次重排更新。
