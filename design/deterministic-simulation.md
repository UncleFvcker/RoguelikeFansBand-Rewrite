...
当前 state hash Schema 为 v9：哈希输入覆盖运行时内容包 ID/hash、world ID、战斗状态、物品、RNG、世界脉冲和命令序号。active contract-v15 的 projectile 轨迹是命令期间的瞬时 outcome；弹药扣减与投掷落点已经体现在现有物品实例、位置和分配器状态中，因此不虚增 state hash Schema。
