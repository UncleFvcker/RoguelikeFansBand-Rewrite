...
- 不包含 `contentHash`；内容匹配由存档与回放头独立执行；
- hash Schema 自身有版本号；
...
v91 的怪物位移：blink/escape 在候选规划阶段零 RNG（行优先规范枚举合法落点，空候选以 no-space 拒绝且不进入权重池），被选中后执行时消费一次有界抽取选落点；escape 的最小距离过滤为空时按减半距离回退一次再拒绝；drag 的落点是施法者八邻规范序第一个合法格，零 RNG，玩家被拖复用 relocate_player 的到达管线。位移只改位置，state hash 沿用 Schema v40。

v92 的新状态族只在状态存在时消费 RNG：混乱移动先抽一次 `bounded(4)`（0 保持原方向），否则再抽一次 `bounded(8)` 按规范方向序重定向；混乱禁施法、致盲 FOV 短路与麻痹的行动替换全部零 RNG（麻痹把推进世界时间的行动换成内部 `ParalyzedIdle`，能量、怪物行动与状态 tick 照常，等待恢复不触发）。三个状态种类写入既有 statuses 域，state hash 沿用 Schema v40。
...
G0 的 `autoGetMode` 是零耗时、零 RNG 的每角色配置。无效墨家名器规则会使整次配置失败，因此模式不会被部分更新。三态值进入 state hash Schema v75；本阶段没有 Ctrl+G 命令、目标、路径或额外运行时状态。

G1 的自动拾取目标选择不消费 RNG。候选先按模式、发现知识、墨家名器动作、销毁保护、投射规则和 O3 本地可达性过滤，再按 RFB 距离与实例 ID 取最小值。目标坐标只存在于快照投影；金币 `discovered` 是新增权威状态，进入 state hash Schema v76。共同哈希结构变化使 470 个 exact fixture 统一刷新至 contract-v218。
...
contract-v279 的 streamer 对每个候选先抽已知富矿；仅在未命中时抽隐藏富矿。玩家成功
挖除矿脉后先写地形、再零 RNG 增长熟练度，随后按材料、富矿金币、额外物品的顺序消费
RNG；材料与金币计算使用增长后的熟练度。魔法摧毁富矿只进入普通金币生成，怪物破墙不
进入玩家收益。碎石掉落在成功移除后按原版几率判定，再复用楼层 loot RNG。
