...
当前 state hash Schema 为 v10：哈希输入覆盖运行时内容包 ID/hash、world ID、战斗状态、物品、按稳定 kind ID 排序的物品知识、RNG、世界脉冲和命令序号。active contract-v21 的消耗品使用只改变已经覆盖的 HP、物品数量与 `ItemKnowledge`，因此不虚增 state hash Schema。
