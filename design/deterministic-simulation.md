...
contract-v55 的 maze 将格点按行优先编号，从抽取根节点开始，以固定 `north/east/south/west` 候选执行随机 DFS；destroyed 从规范中央候选抽取震中，再沿 `y/x` 四向前沿扩展；streamer 依次抽取权重、普通墙起点、八向方向和规范候选，空间不足时只从剩余 wall 回退。v54 已生成楼层及 RNG 原样迁移，不补建 maze/destroyed/streamer；state hash 继续使用 Schema v19。
