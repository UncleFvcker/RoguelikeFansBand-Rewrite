...
当前 state hash Schema 为 v9：哈希输入覆盖运行时内容包 ID/hash、world ID、战斗状态、物品、RNG、世界脉冲和命令序号。active contract-v14 的 projectile 轨迹是命令期间的瞬时 outcome，发射器 profile 由内容与装备实例派生，因此不虚增 state hash Schema。
