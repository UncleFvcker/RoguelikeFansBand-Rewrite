...
object、actor、visibility 和 lighting 使用按尺寸分组的动态 chunk view。后端始终保存最新 `RenderCell` 语义数据，但只为可见 chunk 挂载逐格 Pixi 对象；玩家居中模式根据相机偏移、缩放和 420×420 视口，把屏幕矩形转换回未缩放世界坐标，并额外保留一格 overscan。镜头跨 chunk 时先释放离开视口的 view，再把同尺寸 view 重新定位并用目标 chunk 的语义数据刷新。每种边缘尺寸最多保留一个空闲 view，避免从整图切回玩家居中后继续常驻全部对象。
...
后端 ID 升级为 `pixi-layered-chunks-v3`，动态视图模式为 `visible-chunk-reuse-v1`。诊断还包含 active/pooled dynamic chunk 数、实际 cell view 数和实际动态 display object 数。这些属性只用于测试和性能分析，不进入游戏协议或规则状态。
...
- 不可见 chunk 仍同步语义数据，但不再预分配或提交逐格动态 display object；
- 真正的地形动画需要进入独立 effects pass，不能每帧使静态 chunk 失效；
- 整图滚动模式尚未按滚动矩形剔除；
- 可见 chunk 动态视图复用已经实现；整图模式尚未监听滚动矩形，因此仍会挂载全部 chunk；低分辨率 lighting RenderTexture 和 Debug pass 后续再评估。
