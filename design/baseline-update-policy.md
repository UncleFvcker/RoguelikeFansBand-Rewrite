...
`tests/fixtures/contract-v72/baseline-policy.json` 是当前 active 机器可读政策。`contract-v1` 至 `contract-v71` 保留为历史基准。公共 CI 每次运行：
...
真实 `.rfbcontent` 激活已经按此规则建立 `contract-v2`；后续每个协议/内容/state hash 迁移都保留独立历史目录。当前规则边界为 `contract-v72`：device/saving-throw/stealth/perception 的权威消费、结构化 check outcome、actor 警戒存档兼容和 160 个 exact fixtures 由 [Contract v72](contract-v72-observable-skill-checks.md) 定义；[Contract v71](contract-v71-rfb-character-builds.md) 及 `contract-v1` 至 `contract-v71` 继续作为历史基准保留。
