...
真实 `.rfbcontent` 激活已经按此规则建立 `contract-v2`；后续每个协议/内容/state hash 迁移都保留独立历史目录。当前规则边界为 `contract-v103`：动态 affix 实例、装备加值/passive、save/replay、state hash Schema v42 和 328 个 exact fixtures 由 [Contract v103](contract-v103-dynamic-affixes.md) 定义；`contract-v1` 至 `contract-v102` 继续作为历史基准保留。
