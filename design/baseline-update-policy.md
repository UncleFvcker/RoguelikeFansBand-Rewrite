...
`tests/fixtures/contract-v74/baseline-policy.json` 是当前 active 机器可读政策。`contract-v1` 至 `contract-v73` 保留为历史基准。公共 CI 每次运行：
...
真实 `.rfbcontent` 激活已经按此规则建立 `contract-v2`；后续每个协议/内容/state hash 迁移都保留独立历史目录。当前规则边界为 `contract-v74`：等待/休息资源恢复、自身目标、治疗能力、结构化中断、旧存档兼容和 174 个 exact fixtures 由 [Contract v74](contract-v74-resource-recovery-and-healing.md) 定义；[Contract v73](contract-v73-ability-books.md) 及 `contract-v1` 至 `contract-v73` 继续作为历史基准保留。
