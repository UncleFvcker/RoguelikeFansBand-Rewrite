...
`tests/fixtures/contract-v105/baseline-policy.json` 是当前 active 机器可读政策。`contract-v1` 至 `contract-v104` 保留为历史基准。公共 CI 每次运行：
...
真实 `.rfbcontent` 激活已经按此规则建立 `contract-v2`；后续每个协议/内容/state hash 迁移都保留独立历史目录。当前规则边界为 `contract-v105`：Death 第二册、职业 beam 档案、灭绝、临时品牌、吸血、尸体/复活、save/replay、state hash Schema v44 和 343 个 exact fixtures 由 [Contract v105](contract-v105-death-second-book.md) 定义；`contract-v1` 至 `contract-v104` 继续作为历史基准保留。
