...
`tests/fixtures/contract-v80/baseline-policy.json` 是当前 active 机器可读政策。`contract-v1` 至 `contract-v79` 保留为历史基准。公共 CI 每次运行：
...
真实 `.rfbcontent` 激活已经按此规则建立 `contract-v2`；后续每个协议/内容/state hash 迁移都保留独立历史目录。当前规则边界为 `contract-v80`：RFB 式定点/实体延长射线的目标验证、稳定整数斜率、actor 穿透、墙体截断、共享单次伤害骰、空射/无效目标 RNG 边界、能力事件和 202 个 exact fixtures 由 [Contract v80](contract-v80-targeted-beam-extension.md) 定义；[Contract v79](contract-v79-cone-damage.md)、[Contract v78](contract-v78-beam-damage.md)、[Contract v77](contract-v77-area-damage.md) 及 `contract-v1` 至 `contract-v77` 继续作为历史基准保留。
