...
`tests/fixtures/contract-v81/baseline-policy.json` 是当前 active 机器可读政策。`contract-v1` 至 `contract-v80` 保留为历史基准。公共 CI 每次运行：
...
真实 `.rfbcontent` 激活已经按此规则建立 `contract-v2`；后续每个协议/内容/state hash 迁移都保留独立历史目录。当前规则边界为 `contract-v81`：短距 teleport 的 position 落点验证、可见性/line of effect、可行走与 actor 占用拒绝、零资源/零 RNG 前置边界、精确移动、能力事件和 209 个 exact fixtures 由 [Contract v81](contract-v81-teleport-ability.md) 定义；[Contract v80](contract-v80-targeted-beam-extension.md)、[Contract v79](contract-v79-cone-damage.md)、[Contract v78](contract-v78-beam-damage.md)、[Contract v77](contract-v77-area-damage.md) 及 `contract-v1` 至 `contract-v77` 继续作为历史基准保留。
