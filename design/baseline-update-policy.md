...
`tests/fixtures/contract-v106/baseline-policy.json` 是当前 active 机器可读政策。`contract-v1` 至 `contract-v105` 保留为历史基准。公共 CI 每次运行：
...
真实 `.rfbcontent` 激活已经按此规则建立 `contract-v2`；后续每个协议/内容/state hash 迁移都保留独立历史目录。当前规则边界为 `contract-v106`：Death 第三册、随机效果、随机状态时长、状态派生加值、重复追踪吸血、全可见目标伤害、永久武器 affix、save/replay、state hash Schema v45 和 353 个 exact fixtures 由 [Contract v106](contract-v106-death-third-book.md) 定义；`contract-v1` 至 `contract-v105` 继续作为历史基准保留。
