...
`tests/fixtures/contract-v108/baseline-policy.json` 是当前 active 机器可读政策。`contract-v1` 至 `contract-v107` 保留为历史基准。公共 CI 每次运行：
...
真实 `.rfbcontent` 激活已经按此规则建立 `contract-v2`；后续每个协议/内容/state hash 迁移都保留独立历史目录。当前规则边界为 `contract-v108`：治疗骰、实例级充能、设备成功/失败/耗尽事务、知识门控、save/replay、state hash Schema v47 和 368 个 exact fixtures 由 [Contract v108](contract-v108-charged-items.md) 定义；`contract-v1` 至 `contract-v107` 继续作为历史基准保留。
