...
已实现。实际来源为 RFB `master` Git 对象 `a0d92b6378d148c5262cc236b8fa6ed2ca06a54c`：
`object2.c::_make_object_aux/make_object/acquirement/drop_near`、`apply_magic` 和 `obj.c::obj_can_combine/obj_combine`。

单次选择和物化返回草稿或正常失败；完整生成按 Ordinary 1 次、无显式 hook 的 Good/Great
100 次、Tailored 1000 次运行。GreatOnly 属于 AM_GREAT；现有 Artifact 模式的源调用同时带
AM_GOOD/AM_GREAT。来源基础池和主题均不算调用前已设置的显式 hook；现有固定候选表保持
指定池入口，不进入来源类别选择。Craft 和固定奖励仍调用自己的物化入口。
瞬时固定神器在空底材池判断之前尝试；装置物化失败也作为一次正常失败，不回填通用 Ego。

Acquirement 每次只抽一次 1 或 2–3 的目标数量，外层最多调用 1000 次完整生成。
成功生成便扣减数量，再执行落地；无空间消失不补发。移除了卷轴预检中的静态装备池限制，
空池仍正常消费并使卷轴种类 aware；真实 ID 耗尽在命令修改状态前返回错误，保留原有最大数量预检。

| 阶段 | 固定神器 generated | 随机神器名称 | 实例 ID | 知识 / 书本 found |
| --- | --- | --- | --- | --- |
| 底材或装置生成失败 | 尚未物化则不登记 | 保留工厂已登记的名字 | 不分配 | 不提交 |
| 完整神器草稿被 Tailored 拒绝 | `apply_magic` 阶段登记并保留 | 保留，沿用 E8.5 | 不分配 | 不提交 |
| 草稿接受并落为新物品 | 保留 | 保留 | 此时分配 | 奖励不自动鉴定；落地不记 found |
| 完整合入已有可堆叠实例 | 不适用于神器 | 不适用于神器 | 使用已有 ID | 沿用已有知识，书本仍按 B1 单件边界 |
| 落地无空间 | 对未鉴定固定神器解除登记，采用源默认 preserve 行为 | 保留 | 不分配 | 不提交；卷轴仍消费并 aware |

所有正常失败保留 RNG；没有整局复制回滚。奖励来源记为 Acquire，事件列出实际落地的目标 ID，
零物品也发出正常 Acquirement 事件。改变格集合使用实际落点，拾取/鉴定后的 found 仍走 B1。

落地复用物品实例的现有堆叠相容性，并与造物能力共用一条实现：半径三且距离平方不超过十，
检查地图内部、投射路径和可落地地形，计入金币堆，以距离/堆数评分并对同分格作源端随机选择。
普通物品即使 chance=-1 也保留破碎检查的 RNG；99 堆检查区分可合并与新堆，源端满堆仍可参与
相容性评分的行为也保留。神器附近无位置时随机游走最多 1000 次，再在全图合法格中抽取。
布帘的 projectable/item-drop 标记独立于 walkable；楼梯、门、符文等禁放格不再因为可步行而接收奖励。
当前物理书本仍沿用 B1 的单件约束，未扩展为可混合来源和 counted 状态的书本堆；不把现有实例
表示宣称为完整 `obj_combine` 对象模型。原版瓶子转化/捕获球释放在当前 Tailored 候选和造物入口不可达。
