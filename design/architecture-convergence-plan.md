...
Status: phases 0-14 are complete and Phase 15 is active. Monster ability selection and selected execution are separated, all abilities require source-only ability programs and casting-policy bindings, the legacy importer emits both roots, and all inline ability-effect/player-policy source compatibility paths have been removed. Phase 15 Gates 1-8 are complete; Gate 9 consolidation and acceptance is next. The implemented boundary covers protocol projection, validation, persistence orchestration, test organization, pure world-generation calculations, inventory, terrain, floor-lifecycle and recall plan/commit operations, frontend composition, character progression, task/campaign transition plans, aggregate-local damage, death, attack-family resolution, player abilities, item use, monster abilities, and the content definition/artifact/schema/catalog/source/validation pipeline.
