...
Status: phases 0-15 are complete and Phase 16 is active. Monster ability selection and selected execution are separated, all abilities require source-only ability programs and casting-policy bindings, the legacy importer emits both roots, and all inline ability-effect/player-policy source compatibility paths have been removed. Phase 16 is reducing the remaining `Game` implementation-module responsibility mixing without replacing the authoritative aggregate root. The implemented boundary covers protocol projection, validation, persistence orchestration, test organization, pure world-generation calculations, inventory, terrain, floor-lifecycle and recall plan/commit operations, frontend composition, character progression, task/campaign transition plans, aggregate-local damage, death, attack-family resolution, player abilities, item use, monster abilities, and the content definition/artifact/schema/catalog/source/validation/error pipeline.
...
At the initial audited commit, `crates/rfb-core/src/game/mod.rs` contained 26,254 lines. After Phase 15 it contains 11,601 lines, while 20 private production modules now own snapshot projection, persistence, validation predicates, inventory, progression, task/campaign planning, terrain and floor lifecycle, damage/death and attack families, player/item/monster ability execution, status effects, and pure world-generation calculations. `Game` is the correct authoritative aggregate root, but its implementation module still owns several unrelated change domains: authoritative world-generation orchestration, loaded-state validation orchestration, player casting/resource policy, equipment-derived stats and item-knowledge projection, and turn/movement coordination.
...
### Phase 15: content compilation pipeline boundaries (completed)
...
The character-validation sub-batch moves skill, skill-set, race, class, personality, build, starting-item, body-slot, casting-profile, technique-profile, and device-recharge checks into the private 565-line `validation/characters.rs`. One explicit definitions carrier lists the six mutable character families, and one domain-specific reference carrier lists the item, terrain, actor, ability, book, resource, and deferred race references the checks consume; neither provides generic lookup or registration behavior. The module retains the original skill-to-build order and returns only the build IDs required by later world validation. The parent falls to 4,365 lines. Both content feature matrices, schema freshness, source verification with the canonical demo hash, workspace all-target checks, zero-warning Clippy, all 33 replay tests, and all 454 fixtures pass.

The ability-validation sub-batch moves resource, ability, ability-book, effect/target/scaling, and actor-casting compatibility checks into the private 840-line `validation/abilities.rs`. Three explicit mutable definition slices and six established lookup sets/maps plus the actor module's deferred casting candidates are the complete inputs; seven typed outputs retain resource, ability, book, corpse-item, and race-reference data for the later item and character calls. The compatibility pass reads each actor-owned `monsterCasting` definition but does not own or rewrite its frequency, tactics, candidate order, IDs, or weights. The parent falls to 3,622 lines. Both content feature matrices, schema freshness, source verification with the canonical demo hash, workspace all-target checks, zero-warning Clippy, all 33 replay tests, and all 454 fixtures pass.

The item-and-table validation sub-batch moves item-effect shape/target rules, item/device/equipment/projectile/corpse checks, and the Effect Program-facing item-effect entry point into the private 747-line `validation/items.rs`; the latter remains only `pub(crate)`. It separately moves loot, encounter, vault, theme, region, and terrain-feature table validation into the private 669-line `validation/tables.rs`. Item validation still runs before character validation and returns only item limits; table validation still runs after character validation and returns the six maps/sets consumed by worlds. Actor loot references and ability/actor corpse references preserve their original deferred-check positions. The parent falls to 2,365 lines. Both content feature matrices, schema freshness, source verification with the canonical demo hash, workspace all-target checks, zero-warning Clippy, all 33 replay tests, and all 454 fixtures pass.

The terrain-affix-and-world validation sub-batch completes Gate 7. Terrain schema, transition, concealment, trap, walkability, connectivity, tag, and open-target checks move to the private 205-line `validation/terrain.rs`; affix modifier, roll-group, status-immunity, and tag checks move to the private 84-line `validation/affixes.rs`; task, dungeon, procedural-floor, generation-budget, connection, spawn, and world-instance checks move to the private 1,902-line `validation/worlds.rs`. The 242-line parent now owns only metadata checks, deterministic root sorting, the original terrain-to-world orchestration order, and explicit typed data flow between validators. No generic validation context, registry, trait, dynamic dispatch, or new public API is introduced. Both content feature matrices, schema freshness, demo source verification, the real fixed-source import, workspace all-target checks/tests, zero-warning Clippy, all 33 replay tests, and all 454 contract fixtures pass. The canonical hashes remain `cf977b882f1650f641035e1e12b22cca6430106a4992cceefd2e496060f51774` and `54333ae2cda9df63ceaccc23794f54a66033897630afe44aa2f845fb217807ad`. Gate 8 definition ownership is next.

The first definition-ownership sub-batch creates the private `definitions/` boundary and moves the two pack metadata definitions, eleven actor/stat definitions, and seventeen character/build definitions into 28-line `pack.rs`, 232-line `actors.rs`, and 297-line `characters.rs`. `definitions/mod.rs` re-exports these types only to the crate root, so every established `rfb_content::*` path remains unchanged and no public submodule hierarchy is added. Cross-family inputs are explicit: character definitions import the actor resistance/stat types and the existing ability-scaling definition without introducing a generic shared model. `lib.rs` falls from 2,928 to 2,396 lines while the crate-wide public definition census remains 138. Both content feature matrices, schema freshness, demo source verification, workspace all-target checks, zero-warning Clippy, all 33 replay tests, and all 454 contract fixtures pass with the canonical demo hash unchanged. Ability and item definitions are the next Gate 8 sub-batch.

The ability-and-item definition sub-batch moves seventeen resource/ability/effect definitions into the private 664-line `definitions/abilities.rs` and twenty-four affix/equipment/item definitions into the private 595-line `definitions/items.rs`. Their intentional type dependency is explicit: ability status effects reuse item brand definitions, while item use definitions reuse ability target/detection types; both reuse actor damage, resistance, stat, and equipment-bonus types. The existing `valid_ability_level_scaling` sibling-validator entry retains crate-internal visibility through the private definitions facade, matching its former crate-root reach without creating public API. `lib.rs` falls from 2,396 to 1,152 lines and no longer imports ordered collections, while all 138 public definition types retain their established crate-root paths. Both content feature matrices, schema freshness, demo source verification, workspace all-target checks, zero-warning Clippy, all 33 replay tests, and all 454 contract fixtures pass with the canonical demo hash unchanged. Table and world definitions are the final Gate 8 ownership sub-batch.

The table-and-world definition sub-batch completes Gate 8 production movement. Twenty-seven loot, encounter, theme, region, terrain-feature, and vault definitions move into the private 333-line `definitions/tables.rs`; thirty-nine terrain, fixed-world, campaign, dungeon, task, floor, connection, and procedural-generation definitions move into the private 582-line `definitions/worlds.rs`. The only cross-family links are explicit typed fields: tables consume world positions/terrain overrides, while world item spawns consume table-owned item quality. `lib.rs` falls from 1,152 to 248 lines and now owns constants, private module declarations, stable root re-exports, the established crate-internal ability-scaling entry, and `ContentError`; all serde/Schema definition derives have left the facade and the public definition census remains 138. Both content feature matrices, schema freshness, demo source verification, workspace all-target checks, zero-warning Clippy, all 33 replay tests, and all 454 contract fixtures pass with the canonical demo hash unchanged. A complete Gate 8 acceptance and dependency/public-visibility audit follows before Gate 9.

Gate 8 final acceptance confirms all seven definition-family modules are private, no `pub mod` hierarchy exists, all 138 serializable public types remain reachable from `rfb_content::*`, and the only non-public facade entry remains the previously crate-root-scoped ability-scaling validator. The complete workspace test matrix passes, including 34 content, 307 core, 28 legacy-import, 33 replay, all 454 contract fixtures, and 14 Tauri tests; the 35-test schema feature matrix also passes. Protocol bindings and all 24 generated content schemas are current, zero-warning workspace Clippy passes, and the frontend passes 54 tests, strict TypeScript checking, and the production UI build. The demo pack remains `cf977b882f1650f641035e1e12b22cca6430106a4992cceefd2e496060f51774`. A fresh fixed-source import produces 1,260 abilities, 1,260 Ability Programs, 1,260 casting bindings, and 844 actors with actor-owned `monsterCasting`, and compiles to the unchanged hash `54333ae2cda9df63ceaccc23794f54a66033897630afe44aa2f845fb217807ad`. Gate 8 is complete. Gate 9 next moves the remaining `ContentError` ownership if needed to make the 248-line crate root a strict constants/module/re-export facade, removes only proven dead imports/helpers, audits the final dependency and visibility graph, and runs the Tauri release/desktop E2E acceptance.

The Gate 9 consolidation sub-batch moves the unchanged public `ContentError` enum and its path/error dependencies into the private 188-line `error.rs`, then re-exports the enum from the crate root. The test root now imports `PathBuf` explicitly instead of inheriting that implementation dependency through `use super::*`. The resulting 63-line `lib.rs` contains only private module declarations, stable public/crate-internal re-exports, the 26 established format/schema constants, and the private test declaration. Focused default/schema content tests, all-target compilation, and zero-warning Clippy pass; the full release acceptance remains the final Gate 9 step.

Gate 9 final acceptance confirms the private dependency graph and stable crate-root facade with no new public module hierarchy, API, error variant, or helper visibility. Formatting, protocol binding generation, all 24 content schemas, demo source verification, workspace all-target compilation, zero-warning Clippy, and the complete Rust test matrix pass: 34 content, 35 schema-feature content, 307 core, 28 legacy-import, 33 replay, all 454 contract fixtures, and 14 Tauri tests. A fresh fixed-source import retains 1,260 abilities, 1,260 Ability Programs, 1,260 casting bindings, and 844 actor-owned `monsterCasting` definitions. The demo and fixed-source hashes remain `cf977b882f1650f641035e1e12b22cca6430106a4992cceefd2e496060f51774` and `54333ae2cda9df63ceaccc23794f54a66033897630afe44aa2f845fb217807ad`. The frontend passes all 54 tests, strict TypeScript checking, and the Vite production build; the Windows Tauri release `--no-bundle` build and debug WebDriver desktop E2E also pass. Gate 9 and Phase 15 are complete without canonical runtime-byte, protocol, save, replay, content-hash, or behavior changes.

### Phase 16: remaining `Game` implementation ownership (active)

- Purpose: reduce the remaining change blast radius in `game/mod.rs` while retaining `Game` as the sole authoritative aggregate root and preserving its external API.
- Current baseline: `game/mod.rs` is 11,601 lines. It declares 20 private production modules plus the private test tree; the established public surface remains constructors, `dispatch`, debug controls, identity queries, save restoration/projection, `state_hash`, `snapshot`, and `load_built_in_content`.
- Aggregate ownership that must remain: authoritative state, command preflight and transaction ordering, RNG state and draw order, entity/item ID allocation, deterministic commit order, revision/sequence/turn advancement, and public event/update/save/hash projection.
- Not in scope: manager/service objects, a generic domain context, traits or registries, a public module hierarchy, protocol/content/save/hash changes, gameplay or generation algorithm changes, moving RNG or ID authority out of `Game`, or a target line count as an end in itself.
- Compatibility: every gate preserves command results, first-error precedence, RNG counters, IDs, ordered collections and events, terrain/entities/items/connections, snapshots, saves, state hashes, replays, fixtures, generated files, and both canonical content hashes.

Phase 16 uses independently committed and pushed gates:

1. **Boundary record and characterization.** Refresh the post-Phase 15 line/module/public-surface baseline, record remaining ownership and rollback rules, and move no production code.
2. **World-generation implementation ownership.** Move the generated-room/vault/pit/feature/region implementation types, authoritative procedural-floor orchestration, floor-connection resolution and placement, room/pit/cavern/lake/river/maze/destroyed-region/streamer generation, encounter placement, spatial vault selection, and terrain-feature placement into `world/generation.rs`. `Game` retains RNG/ID/state authority; no wrapper manager or duplicate path remains.
3. **Loaded-state validation ownership.** Move `validate_loaded_state`, actor validation, and summon-identity validation into `validation.rs` without changing validation or first-error order.
4. **Player casting and resource policy.** Separate build-granted ability state, learning/proficiency/cooldown policy, resource maxima/recovery/decay, and rest resolution from effect execution. Do not enlarge the already effect-focused `abilities.rs` indiscriminately.
5. **Derived stats and item knowledge.** Separate equipment-derived combat/stat calculations from knowledge-masked item projection, using private modules and only the minimum sibling visibility required by existing consumers.
6. **Turn and movement coordination.** Extract bounded status/summon/monster scheduling and movement decisions only where `Game` can remain the transaction and commit owner; leave genuinely cross-domain coordination at the aggregate root.
7. **Consolidation and acceptance.** Remove only proven superseded imports/helpers, audit visibility and dependencies, refresh ownership evidence, and run the complete Rust, generator, source verification, fixed-source import, frontend, Tauri release, and desktop E2E matrix.

Gate 0 records the 11,601-line baseline and the existing 567-line `world/generation.rs` plus 312-line `world/geometry.rs`. Gate 1 is bounded to procedural-floor generation; carried/death loot remains with its current runtime owners. Exact generated floor state, RNG and allocation counters, representative seeded snapshots/hashes, replay checkpoints, and contract fixtures are rollback boundaries.

The Gate 1 ownership move transfers the seven generated room/vault/pit/terrain-feature/region implementation types, the complete authoritative procedural-floor transaction, and its room, connection, pit, hydrology, encounter, vault, terrain-feature, maze, destroyed-region, and streamer helpers into `world/generation.rs`. The module grows from 567 to 3,405 lines while `game/mod.rs` falls from 11,601 to 8,765 lines. `Game` still owns and directly mutates the same RNG, ID counters, content/state collections, and floor result; the floor-lifecycle caller and existing white-box generation tests receive only `pub(in crate::game)` access, while helpers no longer crossing that boundary are file-private. Carried/death loot and non-generation runtime behavior remain in their prior owners. All 31 focused generation tests, 20 world tests, 307 core tests, 33 replay tests, and 454 contract fixtures pass, together with formatting, binding/schema freshness, demo source verification at hash `cf977b882f1650f641035e1e12b22cca6430106a4992cceefd2e496060f51774`, workspace all-target compilation, zero-warning Clippy, and the Tauri all-target check. Gate 1 is complete; Gate 2 loaded-state validation ownership is next.

The Gate 2 ownership move transfers the unchanged 762-line `validate_loaded_state` orchestration plus actor and summon-identity validation into `validation.rs`. The module grows from 287 to 1,053 lines while `game/mod.rs` falls from 8,765 to 7,998 lines. Only `validate_loaded_state` crosses the sibling boundary to persistence with `pub(super)` visibility; actor and summon helpers remain file-private, and the former parent imports of validation predicates are removed. Validation order, exact `CoreError` values, compatibility defaults, content references, collection checks, and the first observable invalid-save result are unchanged. All nine persistence tests and the focused malformed/invalid-state filters pass, followed by all 307 core tests, 33 replay tests, and 454 contract fixtures, formatting, binding/schema freshness, demo source verification at the unchanged canonical hash, workspace all-target compilation, zero-warning Clippy, and the Tauri all-target check. Gate 2 is complete; Gate 3 player casting and resource policy is next.

The Gate 3 ownership move adds the private 721-line `player_abilities.rs` for casting/technique/device profiles, effective ability scaling, learning capacity and book lookup, proficiency and cooldown state, player resource initialization/restoration/recovery/decay, and rest-stop resolution. The `AbilityProgress` representation and its six calculation constants move with that policy; only the type and two constants required by the existing descendant white-box tests retain `pub(in crate::game)` reach. Existing effect execution remains in `abilities.rs`, generic progression capabilities remain in `progression.rs`, and `Game` remains the state and scheduling transaction owner. `game/mod.rs` falls from 7,998 to 7,289 lines. All 64 focused ability tests, 11 progression tests, the resource capability test, all 307 core tests, 33 replay tests, and 454 contract fixtures pass, together with formatting, binding/schema freshness, demo source verification at the unchanged canonical hash, workspace all-target compilation, zero-warning Clippy, and the Tauri all-target check. Gate 3 is complete; Gate 4 derived stats and item knowledge is next.

The first Gate 4 sub-batch adds the private 311-line `item_knowledge.rs` for knowledge-masked modifiers, equipment bonuses, passives, properties, identification, quality, curses, resistances, status immunities, offense, slays, brands, and projectile/throw profiles. Snapshot projection is its only production consumer; the underlying authoritative item/equipment calculations remain temporarily in `game/mod.rs` for the second sub-batch. `game/mod.rs` falls from 7,289 to 6,996 lines. Snapshot and all 39 inventory tests pass, followed by all 307 core tests, 33 replay tests, all 454 contract fixtures, all-target compilation, and zero-warning Clippy. No public API, item knowledge rule, DTO field/order, save/hash representation, or event behavior changes.

At most one phase is active at a time. A phase does not begin until the prior phase passes its complete acceptance matrix.
