...
状态：Fluent 双语基础设施与首批桌面 UI 迁移已完成
...
- 旧源码仅作为本地消息场景参考；新游戏文本从零建立，旧 `localization/` 和旧游戏文本不得自动导入新系统。

## 2. 文本基线原则

`localization/` 中的旧 TSV、旧译文、完成率报告和审阅结果不作为新本地化系统的输入数据，也不用于统计新游戏汉化完成度。新仓库工具默认完全跳过该目录。

本地旧版审计只用于列出需要重新设计的消息场景，扫描范围可以包括：
...
该审计只在本地通过 `RFB_LEGACY_SOURCE` 运行，旧文本、扫描结果和翻译表写入 `.local/`，不提交到新仓库。新游戏的 Fluent 文本和内容名称重新创作，不能批量复制旧 RFB 文本。

旧 `localization/` 目录不读取、不复制、不生成翻译记忆。所有本地审计记录必须包含固定 commit、源码位置、上下文和扫描版本，并只写入 `.local/`。
...
旧 `*_name_zh.inc` 和 `lib/edit/*.txt` 不转换进新仓库。新内容数据重新定义稳定 ID、名称 key 和描述 key；旧存档导入器可以在本机使用独立映射，但映射结果不能成为新游戏内容包。
...
### 阶段 0：本地分析旧消息场景（按需，不主动展开）

- 对固定旧版代码执行一次本地扫描，完全跳过旧 `localization/`；
- 为扫描结果记录源文件哈希、代码上下文和当前 commit；
- 增加状态：未处理、机器初译、人工翻译、语序待改、术语待审、已确认、技术文本；
- 去重并只保留消息类型、参数语义和源码位置，不复制完整旧文案进仓库；
- 把高频运行时消息与低频调试文本分开；
- 建立中文风格指南 v1。

验收：本地报告可以归纳旧版用户消息场景，但新仓库中没有旧文案、旧翻译表或 stale 扫描产物。
...
为命中、暴击、锋锐、杀戮、烙印、抗性、法术、状态、怪物行动、物品感知、鉴定和任务事件重新编写消息。这一阶段从一开始就避免英文式碎片拼接，按语义拆分事件。
...
> 建立 Fluent 双语框架，根据 20 类典型语义场景重新编写结构化消息，并证明英文和中文可以使用完全不同的参数排列。
...
- 已建立原创 `locales/en-US`、`locales/zh-CN` 和简体中文风格指南，未读取旧 `localization/`；
- 简体中文为默认语言，英文作为兼容资源与技术回退；
- TypeScript `@fluent/bundle` 与 Rust `rfb-localization` 共用同一套 FTL；
- 主界面、消息历史、背包、内容名称、按键提示、连接状态和 tileset 提示已迁移；
- `GameEventDto` 继续只传稳定 key/参数，语言切换会重新格式化已有消息；
- 自动测试检查英中 key/参数一致、Fluent 解析、英文回退、中文独立语序和高置信硬编码回归；
- 桌面 E2E 已证明语言切换不改变 state hash、回合或地图 Canvas。

范围决定：现有旧 RFB 已基本完成中文化，不再主动开展大规模旧文本扫描、翻译重做或旧译文迁移。以后只在开发和玩家测试中发现实际可见英文时按场景修正；旧 `localization/` 仍不作为新资源输入。

下一步：

1. 新增功能时同步提供英中 Fluent key，避免产生新的玩家可见硬编码；
2. 实际发现可见英文时补齐对应 key、中文语序和自动测试；
3. 让内容视觉目录输出 `nameKey`，移除前端临时 kind ID → key 映射；
4. 随背包、装备、丢弃和设置功能扩展对应消息；
5. glossary-check、伪语言和旧版本地场景扫描仅在出现实际需求时再启动。
