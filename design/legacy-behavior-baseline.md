...
└─ scenarios/
   ...
   ├─ ...
   ├─ ...
   └─ 26-equip-unavailable-item.json
```
...
每个场景使用稳定 ID。当前 active `contract-v4` 的输入部分包含：
...
`rfb-contract` 执行命令后精确比较最终 revision、turn、command sequence、玩家位置、实体数量、地面物品数、背包堆数、装备数、事件顺序、changed cells、删除实体、结构化错误、state hash 和可选存档回环 hash。提交的 fixture 必须包含断言；`observe` 命令只输出实际观察结果，不会自动改写或批量刷新 golden：

```powershell
cargo run -p rfb-contract -- observe tests/fixtures/contract-v4/scenarios/01-move-north.json
cargo run -p rfb-contract -- verify tests/fixtures/contract-v4/scenarios/01-move-north.json
```
