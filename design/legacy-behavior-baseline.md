...
每个场景使用稳定 ID。当前 active `contract-v7` 的输入部分包含：
...
```powershell
cargo run -p rfb-contract -- observe tests/fixtures/contract-v7/scenarios/01-move-north.json
cargo run -p rfb-contract -- verify tests/fixtures/contract-v7/scenarios/01-move-north.json
```
