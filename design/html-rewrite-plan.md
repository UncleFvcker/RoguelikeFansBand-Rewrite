...
- [Contract v89：友方召唤物行动与首版命令](contract-v89-friendly-summon-commands.md)
...
- 协议 1.89 和 contract-v89 已建立；玩家召唤物进入 actor 能量调度器，使用 Follow/Attack/Keep Distance/Guard 四种零时间全局命令、自身 melee routine 和玩家击杀归属。2 格内召唤物跨层跟随并稳定落位，远处召唤物留层；Web 增加四键命令面板与中英文事件。内容包/hash 保持 1.80.0/`29116f924e1ef4ddf6b0aa43f3b1b1bd0b4d28245ac086bce30d7a008e8e9e8e`，active baseline 共 272 个 exact fixtures、零 waiver，save v1/state hash 升至 Schema v39。
...
- 协议 1.109 和 contract-v109 已建立；P59 增加动态设备 profile/power/目标/成本/随机容量实例、深度过滤与稳定加权选择、目标前置拒绝、damage/detect/heal 激活事件和严格回档。Web 使用 `useTargetSpec` 复用既有目标选择器，未知设备隐藏 profile/充能，自身目标直接发送。demo 新增 Resonance Wand/Staff/Rod；legacy importer 为原版三种通用壳生成首批候选，`device-effect` 64→61。内容包 1.100.0，content hash 为 `8432e5d6b0143608415de0f49969b6445cd902ef4db58c218c347b5da85cabab`，active baseline 共 373 个 exact fixtures、零 waiver，state hash 为 Schema v48。
- 协议 1.110 和 contract-v110 已建立；P60 增加 rod/wand/staff 差异化自然恢复、确定性实例余数、职业 recharge profile，以及资源/设备两类主动充能来源。Web 背包行可直接充能，勾选另一件设备时将其作为来源；结构化事件覆盖不可用、成功、失败与来源损毁。内容包 1.101.0，content hash 为 `f2bf96ea4a980a6a9914ca80dff5527a5e04b2e36d25aa668b118e6562c9cad9`，active baseline 共 379 个 exact fixtures、零 waiver，state hash 为 Schema v49。
...
- 协议 1.115 和 contract-v115 已建立；P65 增加装备/弹药实例强化、原版千分递减概率、+15 上限、神器/堆叠门、非法目标原子拒绝及近战/射击/投掷/护甲派生。Web 显示非零 to-hit/to-damage/to-AC 并格式化结果事件；legacy importer 映射卷轴 sval 16/17/18/20/21，`scroll-effect` 缺口 47→42。内容包 1.106.0，content hash 为 `9bfa2632f2be9129e39a59dad72f7bb9a64fd2f403d74c3feaee1302fb0fe459`，active baseline 共 405 个 exact fixtures、零 waiver，state hash 升至 Schema v51。
- 协议 1.116 和 contract-v116 已建立；P66 增加 normal/heavy/permanent 实例诅咒、武器/护甲施咒、神器 50% 抵抗、普通/强力解除、永久诅咒保留和卸装/替换零时间拒绝。四类物品存档、拆分/堆叠、旧档迁移、知识边界与 Web 投影已贯通；legacy importer 映射卷轴 sval 2/3/14/15，`scroll-effect` 缺口 42→38。内容包 1.107.0，content hash 为 `9d1c6c1e01fb4533aa5a9868f0adfcbe876148d98585412783d0da93f4019dff`，active baseline 共 413 个 exact fixtures、零 waiver，state hash 升至 Schema v52。
- 协议 1.117 和 contract-v117 已建立；P67 增加物品类别召唤 selector、地牢深度/玩家等级来源和 Race `kinCategory`，并复用能力召唤的候选、unique、落位及群体管线。Pet/Kin 以永久 `controllerId` 回档；零候选/零空间正常消费，只记 Tried 且零召唤 RNG。legacy importer 映射卷轴 sval 4/5/6/54，并为 actor/Race 导入 glyph 式 kin category，`scroll-effect` 缺口 38→34。内容包 1.108.0，content hash 为 `0b9023398c8213f9e74d7f0d4d076b8ce70819dbb5cd8cc4eb3a2b84d4996210`，active baseline 共 420 个 exact fixtures、零 waiver，state hash 保持 Schema v52。
...
1. P68 对剩余 `scroll-effect` 34 重新按真实 sval 分组，优先选择可复用现有 terrain/status/actor 管线的世界效果卷轴，跨系统效果继续拆成独立事务；
