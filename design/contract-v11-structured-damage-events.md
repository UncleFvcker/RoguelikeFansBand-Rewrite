...
contract-v10 已能正确结算物理、元素、状态和抗性伤害，但规则事件只向协议投影最终伤害字符串。contract-v11 保留完整 `DamageOutcome`，让本地化、动画、诊断、统计和未来知识发现共享同一份权威结算结果，不再从消息文本或最终 HP 反推。

本切片不增加新的伤害类型、角色属性或检定规则，也不改变存档、RNG、回合顺序和 state hash。
...
1. 为酸、电、冷、毒增加实际规则入口；
2. 建立带来源的 `DerivedStatsPipeline`；
3. 建立通用检定上下文与结果；
4. 在检定接口上实现眩晕能力削弱与恐惧行动限制。
