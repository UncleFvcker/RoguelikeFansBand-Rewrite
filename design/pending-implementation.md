...
状态：基于 contract-v1–v78、前端目标模式和系统路线书审计；每完成一个纵切后同步更新
...
| P19 | 锥形能力效果 | 下一候选 | 复用目标验证与伤害管线，固定八向锥形 footprint、墙体遮挡、目标顺序和事件/RNG 语义；多资源与怪物施法继续后置 |
...
RFB 式方向射线、actor 穿透、墙体截断、近到远顺序、共享单次伤害骰、空射/无效模式 RNG 边界和 Echo Lance 已进入协议 1.78、内容包 1.70.0、save v1 与 state hash Schema v34；active baseline 为 194 个 exact fixtures、零 waiver。详细边界见 [contract-v78](contract-v78-beam-damage.md)。
...
- 自身目标与治疗已由 contract-v74 完成；范围爆发已由 contract-v77 完成，方向射线已由 contract-v78 完成，锥形、定点延长射线、位移、召唤、侦测和地形改变能力仍未实现；
