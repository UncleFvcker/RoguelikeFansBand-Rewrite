...
状态：基于 contract-v1–v57、前端目标模式和系统路线书审计；每完成一个纵切后同步更新
...
| 优先级 | 候选纵切 | 状态 | 边界 |
| --- | --- | --- | --- |
| P0 | 多楼梯与独立到达点 | 待实现 | 多个上下楼梯、稳定连接 ID、到达锚点、shaft 与存档连接语义 |
| P1 | 持久 pack identity 与 AI | 待实现 | 领袖/成员身份，以及首版 seek、surround、guard-leader 行为 |
| P2 | 同层多区域主题 | 待实现 | 区域边界、群落、局部 encounter/loot/theme 与拼接连通性 |
| P3 | 暂停任务管理 | 待实现 | 地表直接放弃、重接次数限制和重新生成策略 |
