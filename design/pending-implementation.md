...
状态：基于 contract-v1–v77、前端目标模式和系统路线书审计；每完成一个纵切后同步更新
...
| P18 | 锥形/射线能力效果 | 下一候选 | 继续复用目标验证与伤害管线，先实现一个 RFB 原版投射形状，固定方向、墙体遮挡、目标顺序和事件/RNG 语义；多资源与怪物施法继续后置 |
...
- 自身目标与治疗已由 contract-v74 完成；范围已由 contract-v77 完成，锥形、射线、位移、召唤、侦测和地形改变能力仍未实现；
