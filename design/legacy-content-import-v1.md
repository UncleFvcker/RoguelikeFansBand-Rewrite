...
地形 180/188；怪物 1332/1396（95.4%），跳过 64 条无可表达近战的条目。1124 条怪物已带完整多段 `meleeRoutine`，107 条仍有无骰副攻（TOUCH/GAZE/WAIL 类）；截至 P43（杂项包）已有 844 条生成 `monsterCasting`，法术映射累计 4601 实例（吐息、召唤、异种元素、心灵、诅咒、杂项 264），抗性档 1023 只怪 3842 条，`FREQ_N` 频率语法已解析，附身组 522 实例入 notApplicableSpells 桶，类型旗标（UNDEAD/DEMON/DRAGON/ANIMAL）折算为 actor 标签，blow 元素名（DISENCHANT/TIME/NETHER 等）直映伤害类型，施法表上限 64、源包文件预算 4096。缺口报告当前优先级（按覆盖数）：S_ 特殊/字形子类 177、DARKNESS 85（房间光照）、ANIM_DEAD 58（尸体）、ANTI_MAGIC 47、TRAPS 44、SHRIEK 42、HAND_DOOM 30、BLINK 变体 29、TELE_LEVEL 25、INVULN 18；blow 效果缺口以 DRAIN_MANA、SHATTER、VAMP、CONFUSE 等效果语义为首（326 实例）；flag 缺口以 BASH_DOOR、DROP_CORPSE、NO_CONF/NO_SLEEP、FORCE_MAXHP、RES_TELE/RES_WALL/HURT_ROCK 为首。

## 4. v2 方向

k_info/e_info/a_info 物品、词条与神器导入已由 [v2](legacy-item-import-v2.md) 完成；字形子类召唤（S_HOUND/S_SPIDER 等）可按 glyph 派生标签；E:/D: 中文名与描述导出为本地 Fluent 片段；装备旗标系统落地后重跑导入提升表达率。
