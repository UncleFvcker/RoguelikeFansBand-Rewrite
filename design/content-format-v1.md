...
角色定义使用必需的 `attack`、`defense`、`maxHp`、`damageDice` 和 `damageSides` 声明基础战斗属性。`attack`/`defense` 是内容评级，由核心稳定映射为近战能力与 AC；伤害骰只接受正整数并逐骰使用权威 RNG。物品定义可使用可选 `equipmentSlot` 稳定字符串声明装备槽，并使用 `modifiers.attack`、`modifiers.defense` 和 `modifiers.maxHp` 声明修正；攻击/防御修正允许正负值，最大生命修正当前要求非负。可装备物品仍要求 `maxStack = 1`；可堆叠物品的拆分由核心持久化实例 ID 分配器处理。原创包 1.4.0 的回声护符使用 `charm` 槽位并提供攻击 +1、防御 +1、最大生命 +4；槽位和属性显示名由 Fluent UI 资源提供，不写入内容实例。
