...
fn random_artifact_throwing_flag_changes_real_throw_range_damage_and_instance_dice() {
    ...
    .unwrap();
    // This seed reaches a 200% critical: the instance's 2d1 becomes 4 before THROWING.
    assert!(events.iter().any(|event|matches!(event,DomainEvent::ItemThrowHit {damage,..} if damage.raw==4*throwing.1/100)),"{events:?}");
}
