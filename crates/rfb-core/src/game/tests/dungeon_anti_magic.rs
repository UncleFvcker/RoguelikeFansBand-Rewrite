...
#[test]
fn dungeon_anti_magic_distinguishes_mindcraft_spells_from_class_powers() {
    // RFB master a0d92b6378, dungeon.c:3959: Mindcraft uses m; clear mind
    // uses U, and Berserker techniques are an explicit m exception.
    let mut mindcrafter = Game::new_with_build(7, "demo.build.mindcrafter").unwrap();
    clear_monsters(&mut mindcrafter);
    mindcrafter.debug_prepare_mindcrafter_e2e(50);
    enter_context(&mut mindcrafter);
    let spells = mindcrafter.snapshot().player.abilities;
    for spell in spells {
        let blocked = spell.id != "demo.ability.mindcrafter-clear-mind";
        assert_eq!(
            mindcrafter.dungeon_blocks_player_ability(&spell.id),
            blocked
        );
        assert_eq!(spell.can_cast, !blocked);
        if blocked {
            assert_eq!(spell.unavailable_reason.as_deref(), Some("anti-magic"));
        }
    }
    choose_human_talent_if_pending(&mut mindcrafter);
    let unchanged = |game: &Game| {
        (
            game.world_tick,
            game.rng_draw_counter(),
            game.player.hp,
    ...
    let before = unchanged(&mindcrafter);
    let events = dispatch_next(
        &mut mindcrafter,
        GameCommand::CastAbility {
            ability_id: "demo.ability.mindcrafter-precognition".to_owned(),
            target: TargetSelection::SelfTarget,
        },
    );
    ...
    assert_eq!(unchanged(&mindcrafter), before);
    let mut berserker = Game::new_with_build(7, "demo.build.berserker").unwrap();
    enter_context(&mut berserker);
    for activation in &berserker
        .content
        .class("demo.class.berserker")
        .unwrap()
        .abilities
    {
        assert!(!berserker.dungeon_blocks_player_ability(&activation.ability_id));
    }
}
