...
fn jewelry_level(rng: &mut RfbRng, spread: u16) -> u16 {
    ...
    let random = if bound <= 1 {
        0
    } else {
        rng.bounded(u64::from(bound)) as u16
    };
    20 + spread / 4 + random
}
...
impl Game {
    fn special_shop_candidate(&mut self, shop: &ShopDefinition) -> Option<ItemInstance> {
        ...
        item.origin_kind = Some(rfb_protocol::ItemOriginKindDto::Shop);
        if shop.category == ShopCategory::Jeweler
            && !accepts_jewelry_artifact(&mut self.rng, artifact)
        {
            return None;
        }
        Some(item)
...
mod tests {
    ...
    fn zul_price_hooks_keep_integer_surcharge_rounding_and_owner_cap() {
        ...
        assert_eq!(
            player_purchase_unit_price(&game, jeweler, 1_234, 140),
            3_460
        );
        assert_eq!(player_sale_unit_price(&game, jeweler, 1_234, 140), 440);
        assert_eq!(
            player_sale_unit_price(&game, jeweler, 1_000_000, 140),
            20_000
        );
        assert_eq!(
            player_purchase_unit_price(&game, dragon, 31_000, 100),
            31_000
        );
        assert_eq!(
            player_purchase_unit_price(&game, dragon, 31_001, 100),
            31_300
        );
        assert_eq!(
            player_purchase_unit_price(&game, dragon, 31_199, 100),
            31_600
        );
        assert_eq!(
            player_purchase_unit_price(&game, dragon, 31_200, 100),
            31_700
        );
    }
    ...
    fn generated_candidates_use_source_hooks_and_allocate_ids_only_when_stocked() {
        ...
        for id in ["demo.shop.zul-jeweler", "demo.shop.zul-dragonskin"] {
            ...
            for _ in 0..512 {
                let Some(item) = game.special_shop_candidate(&shop) else {
                    continue;
                };
