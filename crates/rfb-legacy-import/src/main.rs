...
use rfb_legacy_import::{
    content::{
        ...
        sync_demo_ability_ground_items, sync_demo_armor_ego_identities, sync_demo_item_destruction,
        sync_demo_items, sync_demo_monsters, sync_demo_polymorph_races,
        sync_demo_ranged_ego_identities, sync_demo_ranged_egos, sync_demo_weapon_digger_egos,
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match mode.to_string_lossy().as_ref() {
        ...
        "sync-demo-armor-egos" => {
            ...
            println!(
                "{}",
                rfb_legacy_import::content::sync_demo_armor_egos(&source, &path)?
            );
        }
        "sync-demo-armor-ego-identities" => {
            if args.next().is_some() {
                return Err("sync-demo-armor-ego-identities accepts exactly one pack path".into());
            }
            let source = PathBuf::from(env::var_os("RFB_LEGACY_SOURCE").ok_or(
                "sync-demo-armor-ego-identities requires RFB_LEGACY_SOURCE to point at the legacy repository",
            )?);
            println!("{}", sync_demo_armor_ego_identities(&source, &path)?);
        }
        ...
        "sync-demo-ranged-egos" => {
            if args.next().is_some() {
                return Err("sync-demo-ranged-egos accepts exactly one pack path".into());
            }
            let source = PathBuf::from(env::var_os("RFB_LEGACY_SOURCE").ok_or(
                "sync-demo-ranged-egos requires RFB_LEGACY_SOURCE to point at the legacy repository",
            )?);
            println!("{}", sync_demo_ranged_egos(&source, &path)?);
        }
        "sync-demo-items" => {
        ...
        _ => {
            return Err(
                "mode must be inspect-prefix, record-catalog, verify-catalog, import-content, audit-egos, audit-demo-item-names, audit-demo-items, audit-demo-weapon-proficiencies, audit-demo-monsters, audit-demo-mutations, sync-demo-items, sync-demo-item-destruction, sync-demo-ability-ground-items, sync-demo-monsters, sync-demo-polymorph-races, sync-demo-armor-ego-identities, sync-demo-ranged-ego-identities, sync-demo-ranged-egos, sync-demo-weapon-digger-egos, or sync-demo-wilderness".into(),
            );
        }
    }
    Ok(())
