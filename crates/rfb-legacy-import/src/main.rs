...
use rfb_legacy_import::{
    content::{import_content, sync_demo_items, sync_demo_monsters, sync_demo_wilderness},
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mode = args.next().ok_or(
        "usage: rfb-legacy-import <inspect-prefix|record-catalog|verify-catalog|import-content> <path> | <sync-demo-items|sync-demo-monsters|sync-demo-wilderness> <selection> <output>",
    )?;
    ...
    match mode.to_string_lossy().as_ref() {
        ...
        _ => {
            return Err(
                "mode must be inspect-prefix, record-catalog, verify-catalog, import-content, sync-demo-items, sync-demo-monsters, or sync-demo-wilderness".into(),
            );
        }
    }
    Ok(())
