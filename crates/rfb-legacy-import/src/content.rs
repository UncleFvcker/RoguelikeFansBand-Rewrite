...
pub fn parse_calc_bonuses_defenses(text: &str, hook: &str) -> CalcBonusesDefenses {
    fn literal_adjustment(line: &str, prefix: &str) -> Option<i32> {
        ...
        let (sign, tail) = rest
            .strip_prefix("+=")
            .map(|tail| (1, tail))
            .or_else(|| rest.strip_prefix("-=").map(|tail| (-1, tail)))?;
        tail.trim()
            .trim_end_matches(';')
            .trim()
            .parse::<i32>()
            .ok()
            .map(|amount| sign * amount)
    }
    ...
    let mut see_invisible_minimum_level: Option<u16> = None;
    let mut telepathy_minimum_level: Option<u16> = None;
    ...
    for raw_line in body.lines() {
        ...
        if let Some(minimum_level) = minimum_level {
            if line == "p_ptr->see_inv++;" {
                see_invisible_minimum_level = Some(
                    see_invisible_minimum_level
                        .map_or(minimum_level, |current| current.min(minimum_level)),
                );
                continue;
            }
            if line == "p_ptr->telepathy = TRUE;" {
                telepathy_minimum_level = Some(
        ...
        if let Some(rest) = line.strip_prefix("res_add_immune(") {
            ...
            see_invisible = true;
        } else if line == "p_ptr->telepathy = TRUE;" {
            telepathy_minimum_level = Some(1);
...
fn race_json(
    ...
    if let Some(minimum_level) = entry.see_invisible_minimum_level {
        value["seeInvisibleMinimumLevel"] = serde_json::json!(minimum_level);
    }
...
fn convert_content_from(
    ...
    for entry in egos {
        ...
        *duplicates += 1;
        ...
        if !ego_json_has_substance(entry, &value) {
            *report
                .skip_reasons
    ...
    for (path, text) in &source_objects {
        for (name, body) in extract_race_blocks(text) {
            ...
            if let Some(hook) = entry.calc_bonuses_fn.clone() {
                ...
                entry.see_invisible_minimum_level = see_invisible_minimum_level;
                entry.telepathy_minimum_level = telepathy_minimum_level;
...
fn parse_effect_info_activation_candidates(
    ...
    for (line_index, line) in source.lines().enumerate() {
        ...
        match &mut pending_row {
            Some((_, row)) => row.push_str(line),
        ...
        let Some((row_line_index, row)) = pending_row.take() else {
            unreachable!("completed effect row must exist")
        };
        let after_quote = row
            .strip_prefix("{\"")
    ...
    for ego in egos {
        ...
        *duplicates += 1;
        ...
        entries.push(EgoAuditEntry {
            ...
            craft_type,
            current_importer_expressible: ego_json_has_substance(ego, &value),
...
"#;
        ...
        // ignored; level see-invisible and literal speed, spell capacity and
        // regeneration adjustments count.
        let (
            resistances,
            level_resistances,
            free_act,
        ...
        assert_eq!(see_invisible_minimum_level, Some(40));
        assert_eq!(telepathy_minimum_level, Some(30));
        ...
        assert_eq!(spell_capacity_bonus, 3);
        assert_eq!(
            level_resistances,
            [
        ...
        assert_eq!(conditional_see_invisible_minimum_level, Some(40));
        assert_eq!(conditional_telepathy_minimum_level, None);
        ...
        folk.see_invisible_minimum_level = see_invisible_minimum_level;
        folk.telepathy_minimum_level = telepathy_minimum_level;
        ...
        folk.spell_capacity_bonus = spell_capacity_bonus;
        ...
        assert_eq!(race["seeInvisibleMinimumLevel"], 40);
        assert_eq!(race["telepathyMinimumLevel"], 30);
        assert_eq!(
        ...
        let half_troll = LegacyCharacterEntry {
            id: "half-troll".to_owned(),
            ..LegacyCharacterEntry::default()
        };
        assert_eq!(
            legacy_race_tags(&half_troll),
            [
                "humanoid",
                "legacy-import",
                "rfb-compatibility",
        ...
        let half_titan = LegacyCharacterEntry {
            id: "half-titan".to_owned(),
            ..LegacyCharacterEntry::default()
        };
        assert_eq!(
            legacy_race_tags(&half_titan),
            [
                "humanoid",
                "legacy-import",
                "rfb-compatibility",
        ...
        let dark_elf = LegacyCharacterEntry {
            id: "dark-elf".to_owned(),
            ..LegacyCharacterEntry::default()
        };
        assert_eq!(
            legacy_race_tags(&dark_elf),
            [
                "humanoid",
                "legacy-import",
                "rfb-compatibility",
        ...
        let mindflayer = LegacyCharacterEntry {
            id: "mindflayer".to_owned(),
            ..LegacyCharacterEntry::default()
        };
        assert_eq!(
            legacy_race_tags(&mindflayer),
            [
                "humanoid",
                "legacy-import",
                "rfb-compatibility",
...
"#;
        ...
        assert_eq!(
            ...
            [
                ...
                LegacyInnatePower {
                    ...
                    cost: 2,
                    base_failure_percent: 30,
                    ability_id: "rfb.ability.race.magic-missile".to_owned(),
                },
                LegacyInnatePower {
                    governing_attribute: "intelligence".to_owned(),
                    minimum_level: 5,
        ...
        assert_eq!(
            ...
            "rfb.ability.race.probe-monsters"
        );
        assert_eq!(
        ...
        assert_eq!(
            race["abilities"][11]["abilityId"],
            "rfb.ability.race.magic-missile"
        );
        assert_eq!(
...
"#;
    ...
    fn physical_spellbooks_use_explicit_realm_tvals_without_capturing_necromancy() {
        let life_book = LegacyItemEntry {
            tval: LIFE_BOOK_TVAL,
            sval: LIFE_FIRST_BOOK_SVAL,
            ..LegacyItemEntry::default()
        };
        assert_eq!(
        ...
        let life_third_book = LegacyItemEntry {
            tval: LIFE_BOOK_TVAL,
            sval: LIFE_THIRD_BOOK_SVAL,
            ..LegacyItemEntry::default()
        };
        assert_eq!(
