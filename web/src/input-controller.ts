...
type InputDom = Pick<
  AppDom,
  "mapHost" | "targetCursor" | "targetModeToggle" | "targetModeStatus"
>;
...
export class InputController {
  ...
  render(): void {
    ...
    this.#dom.targetModeToggle.textContent = this.#localization.format(
      this.#state.targeting ? "action-target-cancel" : "action-target-start",
    );
    this.#dom.targetModeToggle.setAttribute(
      "aria-pressed",
      this.#state.targeting ? "true" : "false",
    );
    this.#dom.targetModeToggle.disabled =
      this.#state.busy ||
      this.#state.playerDead ||
      (!this.#state.targeting && !available);
    ...
    this.#dom.targetModeStatus.textContent = this.#localization.format("target-status-active", {
      x: cursor.x,
      y: cursor.y,
      range: spec.range,
    });
  ...
  readonly #handleTargetToggle = (): void => {
    if (this.#state.targeting) this.cancelTargeting();
    else this.startProjectileTargeting();
  };
  ...
  async #confirmTargeting(): Promise<void> {
    ...
    if (!state || !status || !intent || this.#state.busy || this.#state.playerDead) return;
