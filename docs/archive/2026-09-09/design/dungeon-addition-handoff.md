...
> 新一轮接手先读 [三工作树、四对话并行开发交接](parallel-development-handoff-20260909.md)。下文旧 monsters-next 分支及版本属于历史，旧分支已删除；实现方法可参考，完成状态须核对当前 main。
...
使用能准确定位的测试名或模块路径，运行本批行为与直接受影响的既有测试；先复用覆盖，缺少证据时再补。
以下三层是候选检查范围，不要求每个地牢都新增三套完整矩阵；已有通用生成、旅行或存档测试不按地牢复制。
...
从本批实际改变的行为中选择覆盖；复用已有机制时验证必要接入，不重复证明全部通用机制：
