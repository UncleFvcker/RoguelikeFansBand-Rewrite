...
- 从以下候选范围选择本批实际受影响的测试，已有覆盖直接复用，不要求每批新写或全跑：静态数值、
  种族被动、初始美德、能力等级边界、资源支付、临时变形与等级奖励、存档/replay、Web 的 `raceId`。
  未改变持久化或相关运行路径时，不为新增一个种族数值另建存档/replay 测试。
...
遵循当前约定，日常运行本批新增行为及直接受影响的既有测试；先复用覆盖，不要求按内容条目重复新增。
普通合并不自动触发全量 fixture、桌面 E2E 或全量 replay；公共初始化/RNG/共享协议投影/State Hash
输入变化，或明确里程碑/用户要求时，按实际影响扩大范围。检查通过且没有新改动或失败就交付。
具体选择见 [并行交接第 7 节](parallel-development-handoff-20260909.md#7-验证选择)。
