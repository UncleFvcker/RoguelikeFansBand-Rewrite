...
领域日常提交只运行本批行为和直接受影响的既有测试，已有覆盖直接复用。全量 workspace、桌面、Web
和 fixture 不由普通合并自动触发，按实际公共变化或明确里程碑决定，见 [并行交接第 7 节](parallel-development-handoff-20260909.md#7-验证选择)。
以下是候选覆盖范围，按本册实际引入/改变的机制选择，不为每个法术复制完整矩阵，也不要求重测未变的通用机制：
