...
第一、二步完成，核对日期 2026-09-10；已实现职业出生/成长及“头脑清明”，正常入口未开放。对应[职业计划](mindcrafter-class-plan.md)。下文来源矩阵保留第一步审计基线；第二步实施范围和仍存限制见第七节，不把“已有类型”视为原版规则已完成。
...
当前背包、箭袋和包袋没有逐物品容器身份，随身感知统一使用原版背包的三分之一抽样门槛；装备感知保留入口，但当前装备操作本身即完成鉴定，不能声称已有原版的未鉴定装备流程。休息基础回蓝沿用项目现有整数恢复，再加职业恢复量，未重写原版小数再生系统。禁咒/反魔法的心灵法术入口、动态费用、普通装备 EASY_SPELL/DEC_MANA 过滤、特殊神器及失败反噬与 14 项法术一起在第三步完成，不能误套给独立职业能力。
