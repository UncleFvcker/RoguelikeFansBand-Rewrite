...
五步全部完成，核对日期 2026-09-10；已实现职业出生/成长、全部心灵法术、失败链及可达神器关联，正常入口、能力 UI 和约定的职业交付验收均已完成。对应[职业计划](mindcrafter-class-plan.md)。下文“审计主线/现状”描述第一步发现的缺口；第七节记录实际实施、验收范围和仍存限制。
...
| 前端 | 已接入“心智 → 心灵术士”叶子和 Class 职业能力分组，无领域子页；第五步完成正式开局、实际施放、升级投影及保存恢复专项桌面检查。 |
...
第四步复用已经支持正式 Build 的 Rust 出生入口，新增正常创角测试及前端“心智”分类，未增加第二套职业允许集合。能力投影提供不可用原因，UI 复用 Class 来源/分组，只显示已解锁能力及施放按钮；动态名称、费用、失败率和目标仍来自 Rust。来源 master 仍为上述 `a0d92b6`；协议升为 1.243，内容包为 1.404.0，保存格式和 State Hash Schema 不变。26 条 active 契约无需刷新通过。专项 Tauri standalone WebDriver 调试包验证升级后原生保存/加载、双语能力显示和法力不足原因、键盘、390 像素窄屏及英文 200% 缩放；高等级使用真实经验阈值授予入口准备，不表示自然升级试玩。当前入口与复现命令集中于[状态页](status.md)，第五步完整验收仍待实施。

第五步补齐托姆特正式职业出生合并、39/40 级种族感知/免费鉴定与职业 12 法力鉴定优先级、重头饰仅禁用种族来源、知识丢失后种族感知恢复，以及夺心魔 PSI 与职业能力同时投影。普通新角色的桌面链从未改动的出生状态开始，神经爆破成功后移动；3 级通过经验入口准备，预知成功、微级位移自然失败一次后再次施放成功，保存恢复后相同动作产生完全相同的状态哈希。发现标题页刷新会调用未初始化的 Pixi 销毁方法，已按实际 renderer 状态保护；标题页和游戏页刷新无未捕获异常。

交付验收通过：workspace（不含 Tauri）、Tauri 原生层 23 项、前端 197 项、补充职业/托姆特与渲染专项；workspace Clippy、typecheck、格式、协议/内容生成检查、内容锁和 26 条完整契约回放。优化 standalone 由 Tauri `build --no-bundle` 构建，已实际启动到标题/创角页并正常退出；本机原生截图接口报 `SetIsBorderRequired ... 0x80004002`，因此完整交互及截图证据来自同源 WebDriver 调试包。未进行自然练级至高等级、完整通关、Android 或全部原版关联验收；第六节的随机神器生成及其他未实现系统限制仍保留。第五步未改内容、协议或保存格式，版本沿用第四步。交付包与复现命令见[状态页](status.md)。
