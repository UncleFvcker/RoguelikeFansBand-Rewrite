...
拒绝确认不改变领域；确认后取消选择法术，已经改换的领域仍然保留，本次不支付学习机会和行动能量。源还在确认改换后对选中的书运行 autopick；可能发生的物品变更不能被误算为成功学习。改换本身不掷随机数；本项目按实际自动拾取消费者记录额外后果。源 old_realm 供角色报告展示，不是禁止换回的黑名单。换回必须重新学习和成长，不能恢复被清除的副领域进度。
