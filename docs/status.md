...
QUESTITEM Q1：[推进计划](quest-item-plan.md)中的三条狗项圈308／309／310和多彩蜈蚣长靴327已定义并挂接现有怪物的10%／5%命名奖励，保留普通池排除；Q1结束时QUESTITEM定义为30/83。本轮没有新增生产规则或保存类型。用户在Q1期间要求后续到Q5实现结束再统一编译验证，未完成队列已停止。此前内容／源参数／估值／本地化／掉落边界与准备式Fang桌面5步已通过；完整池装备保存综合用例的最后修正未复验，契约／生成审计／Clippy仍待完成，不能标记Q1整体验收通过。已有范围与后续检查见计划。

QUESTITEM Q2：十五件正式内容及现有命名怪物掉落已接入，定义累计45/83，余38件。震地者手套IMPACT补入所持武器近战链，百眼／孔德里激活及杰克无燃料4格照明复用现有消费者。维克那／孔德里保留既有守卫奖励，墨伊蕾保留海洋荒野入口；未开放身份分支仍列条件。源参数和核心回归用例已写，本批未执行编译或验证；Q1／Q2完整验收、lock及源C估值／审计更新统一待Q5。Q3进度见下段。

QUESTITEM Q3：十一件定义及命名奖励已接入，累计56/83，余27件；312因Khorne／Mauler未开放保留条件。补齐冬贝利永久种族门控、每手负攻次、三件普通身份重诅咒构造、风暴使者友军误击，以及九头蛇之眼灯具槽／-7照明／700治疗和阿特拉斯武器IMPACT。新增kind138／623，无普通分配行；乌苟立安特保留先二选一再掷掉率。源参数及核心用例已写，未执行编译、测试、审计或桌面验收；相关验证及lock统一待Q5。Q4进度见下段，边界见[推进计划](quest-item-plan.md)。

QUESTITEM Q4：88『刺针』和342特尔莫拉的熔岩灯已定义并接入特尔莫拉城堡，累计58/83、余25件。金库拾取目标、火山／山下怪物互斥任务、完整三张地图、回城领奖与失败关闭复用已有生命周期；随机图例和神器专用kind626已补齐。366核实为卡罗特门徒被窃神器置换，转入身份条件。源完整城堡前后置尚未移植，本批是金库→互斥战斗任务子集；陷阱及RNG适配见[计划](quest-item-plan.md#q4实现状态与q5待验范围)。用例已写，未执行编译、测试、生成或验收；新类型Schema、lock及相关回归统一待Q5。
...
| 内容包 | 正式包1.465.0；lock仍为1.451.0，按用户要求待Q5统一生成验证 | [pack](../packs/rfb-demo-original/pack.json)、[lock](../packs/rfb-demo-original/content.lock.json) |
...
正式源目录含 14 个 Class、110 个 Build、57 个 Race、36 本能力书、1,915 个 ability、625 个 item、1,410 个 actor、169 个 affix、152 个 mutation。世界定义含 38 个 dungeon（四类随机地牢及北欧神系门控的阿斯加德）；城镇有 7 个 town、69 个 shop、68 个 townFacility。基础分配池为 424 行，固定神器定义映射 188/392；创角开放14职业、103个Build。这些是定义与入口数量，行为验收范围见下文。

常规装备底材：[补完计划](ordinary-equipment-completion-plan.md)五步已完成，新增15件，该批canonical kind共411个，Q3／Q4专用底材补入后当前为414个；有源分配行且非神器专用的常规装备映射为 **201/201（100%）**。死神镰刀已接生成限制、穿透无敌、近战反噬及条件返回反噬；泳装旧护甲店交易已接特殊估值。五步实现结束后统一验证：核心首轮1737通过、7失败；修正泳装旧护甲店估值入口，以及池变化暴露的抽样、唯一神器保留和路线夹具后，7项定向复验全部通过，受清场准备影响的阿斯加德楼层5项亦通过，最终有效1744项通过（全量另6项ignored，其中桌面导出用例随后显式运行通过）。内容首轮171通过、2项旧数量断言失败，更新15件带来的正式数量后2项复验通过，最终有效173项；导入器201项、本地化39项、源C估值298例、26条未刷新active契约、相关Clippy及内容lock检查通过。103个开放Build的生成责任审计无记录缺口；这不是逐Build实战声明。普通Tauri standalone完成7件代表底材的11次装备／行动及原生保存恢复，完整状态哈希一致，截图／报告在 `test-results/ordinary-equipment/`。准备包括授予物品、局部地形与目标、镰刀临时HP及反噬种子，不代表自然取得、练级或Android验收。 kind111仍是未开放身份的内部表示，kind245整数权重0不变；挖掘工具通过完整矮人主题池取得，无主题普通分配按源排除它们。普通镰刀不会自动返回，返回规则只有准备状态的核心证据；泳装及镰刀的未开放身份继续单列，不表示原版全部行为完成。
