...
QUESTITEM Q3：十一件定义及命名奖励已接入，累计56/83，余27件；312因Khorne／Mauler未开放保留条件。补齐冬贝利永久种族门控、每手负攻次、三件普通身份重诅咒构造、风暴使者友军误击，以及九头蛇之眼灯具槽／-7照明／700治疗和阿特拉斯武器IMPACT。新增kind138／623，无普通分配行；乌苟立安特保留先二选一再掷掉率。源参数及核心用例已写，未执行编译、测试、审计或桌面验收；相关验证及lock统一待Q5。下一批Q4，边界见[推进计划](quest-item-plan.md)。
...
| 内容包 | 正式包1.464.0；lock仍为1.451.0，按用户要求待Q5统一生成验证 | [pack](../packs/rfb-demo-original/pack.json)、[lock](../packs/rfb-demo-original/content.lock.json) |
