...
| 协议 | Rust定义1.255；生成绑定/Schema仍为旧版，待战法师第七步结束后同步 | [协议常量](../crates/rfb-protocol/src/lib.rs) |
...
| 内容包 | 1.433.0；lock仍为1.431.0摘要，最终同步/verify待战法师第七步结束后 | [pack](../packs/rfb-demo-original/pack.json)、[lock](../packs/rfb-demo-original/content.lock.json) |
...
正式源目录含 13 个 Class、109 个 Build、57 个 Race、36 本能力书、1,905 个 ability 文件、469 个 item、1,409 个 actor、169 个 affix、152 个 mutation。世界定义含 33 个 dungeon 条目；城镇源目录有 6 个 town、60 个 shop、64 个 townFacility。这些是定义/源文件数量，不是完整规则或已验收内容数量。

战法师已完成[七步计划](warrior-mage-class-plan.md)前三步。8个主奥秘Build和288项参数已定义；双书出生、1—50级成长、INT法力、负重/手套、武器熟练度、近战次数、感知/美德、三法球成长与智能怪物反魔法权重已实现。自主学习/重复研习、84点预算与历史支出校验、主副熟练度、首用经验、遗忘恢复和改换保存已接通；9项新增学习专项及受影响共享调用者共71项通过，真实地牢练习和读档后自然失败/改换/继续施法保持一致。第四步25级双向转换、真实资源结算与中英文日志已写，包含自然失败不扣内部资源、满资源仍付代价、低血量死亡及公共伤害/治疗修正；最终代码尚未验收。用户已要求测试和编译统一留到第七步结束后，当前协议/内容Schema生成及最终lock验证也待该时点完成。第五步已写入秘银链甲、罗恩格林护甲、魅力吊坠及真实激活、旧城堡1:4/重复替代、长剑任务、两塔会员与Mage/20卷轴偏好；新专项和共享8个Build消费者用例尚未执行。基础分配审计与正式入口报告仍待统一生成/核验。第六步普通创角已写入“混合 → 战法师 → 第二领域”8个入口，当前目录为13职业、102个Build。双方书本沿自主学习/重复研习/改换UI；能力页新增双语核心转换量/内部支付/失败后果说明。中英文菜单、键盘/取消/忙碌锁、24/25级能力、390px/200%及待确认/改换后保存续演场景代码已写，尚未运行。40项生成责任输入已登记，带显式待验gap；生成报告仍是上一已验版本，不能当作102入口验收证据。桌面新游戏闭环和交付留第七步。
...
职业与种族目录在 [character-creation.ts](../web/src/character-creation.ts)，提交入口在 [session-shell.ts](../web/src/session-shell.ts)，表单在 [web/index.html](../web/index.html)。当前入口目录提供13个职业、102个构筑、46个种族，其中法师占56个有序领域组合、牧师占24个、战法师占8个；新增战法师UI尚待统一验收：
...
| 战法师（奥秘主领域） | `demo.build.warrior-mage-arcane-<second>` | “混合”下8个副领域入口已写，含工艺；UI/核心后续改动待第七步结束统一验收，尚无本批桌面交付 |
...
当前固定神器定义为 86/392，逐项清单已合并游侠两把弓与阿蒙专属神器。『恐惧面具』铁盔的生成身份分支已接入：战士、骑兵、狂战士获得源随机能力／抗性，其他当前职业获得固有诅咒；法师交叉测试覆盖读档后反魔法实际拒绝施法、不消耗法力／RNG，以及卸下后法力上限和施法恢复。[神器计划 A0–A10](artifact-import-plan.md)已完成本轮约定范围，未开放分支与桌面验收继续单列。定义数量不代表自然获取或完整玩法均已验收，边界见[物品计划 I3](remaining-item-coverage-plan.md#i3固定神器与神器创造入口)。
