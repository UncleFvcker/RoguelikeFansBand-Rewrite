...
核对日期：2026-09-11。当前基于种族职业主线 `080005368`、法术道具 `ea723f342` 与地牢城镇 `bd94fe086` 的集成，另完成法师出生、双领域学习成长、副领域改换保存、职业生成/奖励关联、正式 UI 及新游戏桌面交付。本页记录代码/配置事实和注明范围的验收证据。当前数值以链接的源文件为准。
...
职业与种族目录在 [character-creation.ts](../web/src/character-creation.ts)，提交入口在 [session-shell.ts](../web/src/session-shell.ts)，表单在 [web/index.html](../web/index.html)。当前入口提供 65 个构筑、46 个种族，其中法师占 56 个有序领域组合：
...
Death、Arcane、Sorcery、Armageddon、Nature、Life、Daemon、Crusade 各有四册内容、领域 Build 和相关规则测试路径；法师入口已开放这八领域的 56 个有序组合，高阶法师和圣骑士仍只开放 Death。法师代表组合的实际游玩与测试准备范围见上文，不代表每个组合均已完整通关。
...
当前九构筑的生成审计覆盖 36 条来源条件与每构筑五个范围：[审计输入](../design/generation-build-applicability.json)记录实际入口、实现和测试引用，[生成矩阵](../design/ego-contract-audit.json)由完整来源审计生成并接受只读 CI 检查。本次将决斗者纳入，复用基础熟练度偏好补齐量身武器筛选；同时保留盾牌与 200 重量护甲限制、旧城堡奖励及神器替代。真实新游戏用例覆盖量身生成、装备、负向神器、保存和后续 RNG；创造神器卷轴的实际指令覆盖八个可读卷轴构筑，狂战士继续按职业规则拒绝。九构筑当前可达证据缺口为零；缺失身份/内容、重铸入口和对象表示限制仍单列，全原版完成标记为 false。审计引用存在不代表执行过测试，本轮实际结果见上文验收记录。后续按[内容开发](content-development.md#职业与领域-build-的生成接入)维护审计，交接按[并行协作](parallel-development.md#职业与领域生成审计的交接)执行。
