...
核对日期：2026-09-11。本次集成包含种族职业 `7eac7dd23`、法术道具 `b6ba6466d` 和地牢城镇 `2eded9e9c`，合入法师双领域、工艺/神器/堆叠及奥林匹斯收口。本页区分当前代码/配置与各批注明范围的验收证据。
...
`rfb-contentc inspect-source` 已确认上述数量；235 项不同核心测试、36 项内容、39 项本地化、66 构筑审计与 8 项工具测试、相关 Clippy 和 26 条未刷新的 active 契约通过。范围与保留缺口见[神器计划](artifact-import-plan.md#a10-统一验证结果)：145／322 仅当前身份普通分支，萨鲁曼额外掉落、未开放身份／领域／地点消费者仍保留。没有新增自然练级、桌面或 Android 验收。
...
本次合并后的固定神器定义为 45/392，包含法师旧城堡三件与法术道具分支 42 件，逐项清单已同步。『恐惧面具』铁盔的生成身份分支已接入：战士、骑兵、狂战士获得源随机能力／抗性，其他当前职业获得固有诅咒；法师交叉测试覆盖读档后反魔法实际拒绝施法、不消耗法力／RNG，以及卸下后法力上限和施法恢复。[神器计划 A0](artifact-import-plan.md)已完成，下一批为 A1 光源 2、17。定义数量不代表自然获取或完整玩法均已验收，边界见[物品计划 I3](remaining-item-coverage-plan.md#i3固定神器与神器创造入口)。
...
本轮集成核心全量初测 1,443 项通过、7 项失败、3 项原有 ignored；7 项修正后全部定向复验通过，其中法师／任务／竞技场 84 项相关回归通过。修复光束命中漏算法师练习，测试同时覆盖弹道和光束；其余修正明确不兼容铭文、隔离 instant 神器候选、保持神系与地牢抑制状态一致、区分普通掉落与独立征服奖励，以及竞技场长流程的饱食前置。内容 159 项通过，导入器初测 193 项通过、工艺书来源断言修正后复验通过；本地化 39、协议 7、保存 2、回放 9、Tauri 原生层 23、相关前端 108、生成审计工具 8 项通过。66 个开放 Build 的来源适用性检查、相关 Clippy、绑定／Schema、内容锁、类型和格式检查通过。26 条 active 契约在保留法术道具分支已验证的 RNG 行为后，观察确认只有状态哈希差异才刷新并复验通过；未删除行为断言。各方向已有 standalone 交付与桌面证据仍保留原范围，本轮未重建可玩产物或运行桌面 E2E／Android。
...
当前 66 个开放构筑的生成审计覆盖来源条件与每构筑五个范围：[审计输入](../design/generation-build-applicability.json)记录实际入口、实现和测试引用，[生成矩阵](../design/ego-contract-audit.json)由完整来源审计生成并接受只读 CI 检查。本次合并法师 56 个有序组合与高阶法师工艺，保留决斗者等职业的生成限制、奖励与重复神器替代，并核对恐惧面具的当前职业分支。当前可达的文档证据缺口为零；缺失身份／内容、重铸入口和对象表示限制仍单列，全原版完成标记为 false。审计引用存在不代表执行过测试，本轮实际结果见上文。后续按[内容开发](content-development.md#职业与领域-build-的生成接入)维护审计，交接按[并行协作](parallel-development.md#职业与领域生成审计的交接)执行。
