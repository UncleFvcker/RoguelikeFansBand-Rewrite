...
决斗者已完成[计划](duelist-class-plan.md)前四步：正式无 MP 出生、装备天赋、手动/自动挑战、来源实例相关攻防与成长打击，以及全部八项 HP 能力。普通入口已开放“近战 → 决斗者”，禁止冬贝利出生组合。能力面板显示 Rust 投影的 HP 费用、目标和不可用原因；常驻挑战支持重选/解除，Esc 取消瞄准保留挑战。超距冲锋、30 级传送阻止/跟随和 35 级免费重选的界面已接入保存后的原动作继续。量身分配、武器偏好、龙人变形天赋排除、盗贼/旧城堡奖励、两件固定神器及扫射激活、重复神器替代与公会非会员规则已实现。毒针和 Death Scythe 尚未正式导入，公共适配边界见[来源审计](duelist-source-audit.md)。

第四步前端 204 项、决斗者模块 37 项、野外模块 19 项及本地化 39 项通过，相关 Clippy、类型与格式检查通过。前端测试覆盖四类中途选择的接受/拒绝、忙碌锁和隐蔽目标；Tauri WebDriver 实际验证正式人类创角、标记/取消/解除、超距确认、击杀免费重选、HP 不足和装备失效，并检查中英文、原生键盘、390px 窄屏与 200% 缩放。修复测试发现的野外滚动临时缓存误清挑战，保存后相同选择可确定性继续。复现：在 `web` 执行 `npm run e2e:build` 后运行 `node e2e/tauri.e2e.mjs --duelist-ui`；证据位于 `test-results/duelist-ui/`。8/35/50 级和怪物为显式测试准备，不表示自然练级。第三步的 26 条 active 契约证据保留原范围；本步内容/协议/保存格式未变，未刷新 fixture。第五步自然怪物新开局流程、完整交付回归和优化 Tauri standalone 尚未完成。
...
| 决斗者 | `demo.build.duelist` | 单目标挑战与 HP 战技；规则和界面专项通过，禁止冬贝利出生；第五步桌面交付待完成 |
