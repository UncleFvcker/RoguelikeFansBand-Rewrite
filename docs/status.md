...
B2 已接入 17 类共享类别权重、身体槽位减半、Good 分配层级 +10、增深及最大层级过滤，并替换来源路径的单件装备近似过滤。书本、装置、药水与卷轴可以经过现有物化/拾取/保存链；随机神器保留龙鳞甲的底材激活。主题动态规则、完整 tailored 偏好和奖励重试仍待 B3–B5，因此当前奖励可能少于目标数量。真实入口与未开放条件见[分配计划 B2](../design/base-allocation-acquirement-plan.md#b2共享类别选择与-get_obj_num)。
