...
| 内容包 | 最后已验证 1.422.0；A1–A3 源增量待 A10 统一更新版本／lock | [pack](../packs/rfb-demo-original/pack.json)、[lock](../packs/rfb-demo-original/content.lock.json)、[本轮约定](artifact-import-plan.md) |
...
正式源目录含 10 个 Class、73 个 Build、57 个 Race、36 本能力书、1,902 个 ability 文件、441 个 item、1,406 个 actor、169 个 affix、152 个 mutation。世界定义含 32 个 dungeon 条目；城镇源目录有 6 个 town、60 个 shop、64 个 townFacility。这些是定义/源文件数量，不是完整规则或已验收内容数量。A1 两件光源与专用底材、A2 六件护具及 A3 八件武器已补定义，固定神器定义为 61/392；按用户要求，A1–A10 期间不编译，新增测试及运行性验证集中到 A10 结束后执行。

权威内容统计工具是 `rfb-contentc inspect-source`。上次集成已运行内容编译；A1–A3 增量只有静态统计，当前 lock 不包含新增源内容，不能作为本批编译或行为证据。
