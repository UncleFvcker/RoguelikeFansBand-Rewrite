...
核对日期：2026-09-12。当前物品分支已完成C2注明范围的核心实现；最近集成包含种族职业 `edfcac5fb`、法术道具 `886d8da89` 和地牢城镇 `1ef1b7261`，合入游侠、A1–A10 神器与金字塔土丘。本页区分当前代码/配置与各批注明范围的验收证据。
...
| 最近验证内容包 | 1.432.0 | [pack](../packs/rfb-demo-original/pack.json)、[lock](../packs/rfb-demo-original/content.lock.json) |
...
正式源目录含 11 个 Class、77 个 Build、57 个 Race、36 本能力书、1,903 个 ability 文件、529 个 item、1,407 个 actor、169 个 affix、152 个 mutation。世界定义含 33 个 dungeon 条目；城镇源目录有 6 个 town、60 个 shop、64 个 townFacility。这些是定义/源文件数量，不是完整规则或已验收内容数量。
...
C2已接入196『冷酷之牙』和69／187／177／65／70／99／163，当前正式物品529项、固定神器定义126/392，包／lock为1.432.0。投掷共用路径补齐源命中条件、专用暴击、伤害次序与普通武器骰子／射程；163光球的LOS、透明墙、伤害衰减、半径和黑暗地牢照明例外分别验证。首件来自完整普通池，七件续组按真实底材与完整神器门槛条件化验证，包含装备、投掷、落地、拾回、激活及冷却中保存／后续RNG。伏击所需身份尚未开放；65／70／99额外命名掉落、136／383回旋接回及投掷前恐惧／竞技场、特殊消耗品破碎仍保留明确范围。参数、来源及证据见[神器计划C2](artifact-import-plan.md#各组行为验收)；未新增自然练级、桌面或Android验收。

C2本批273项核心、161项内容、39项本地化测试通过（批量检查加失败修复后的定向复验）；覆盖物品、随机神器、投掷暴击、光照、高阶法师、禁近战与武器Ego激活。源C估值295例、来源／lock核验、70构筑适用性审计、26条active契约、Clippy及格式检查通过；未刷新fixture。普通池扩充后调整两项旧生成测试的抽样种子，保留候选集与生成门槛；随机神器旧伤害预期改为包含投掷暴击。
