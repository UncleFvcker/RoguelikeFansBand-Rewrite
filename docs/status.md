...
| 内容包 | 1.448.0实施版本；lock仍为已验收1.444.0，待底材五步后统一编译同步 | [pack](../packs/rfb-demo-original/pack.json)、[lock](../packs/rfb-demo-original/content.lock.json) |
...
正式源目录含 14 个 Class、110 个 Build、57 个 Race、36 本能力书、1,908 个 ability、589 个 item、1,410 个 actor、169 个 affix、152 个 mutation。世界定义含 34 个 dungeon（阿斯加德入口受出生神系门控）；城镇有 7 个 town、69 个 shop、67 个 townFacility。基础分配池为 423 行，固定神器定义映射 156/392；创角开放14职业、103个Build。这些是定义与入口数量，行为验收范围见下文；新增 I1-A 至 I1-D 十四件尚未编译测试。

常规装备底材：[补完计划](ordinary-equipment-completion-plan.md) I1-A 至 I1-D 已接入4件武器／挖掘工具、7件常规护甲、2件固定附魔护具和性感泳装，canonical kind 共410个；有源分配行且非神器专用的常规装备映射为 **200/201（99.5%）**，余1件。pack版本推进至1.448.0，lock仍保留上方已验收的1.444.0；按用户要求，五步实现结束后统一编译测试、更新lock、源C估值及生成审计，前四批未运行任何编译或测试。201包含既有零整数权重分配行，映射率不表示全部自然可掉落或原版行为已完成，kind111内部镰刀不计入分母。剩余死神镰刀；泳装的未开放身份分支单列在补完记录中。
...
本轮集成（`065f4c93f`／`d4e0dffc9`／`3711c91b3`）：核心全量初测1,719项通过、16项失败、5项既有ignored（含两个桌面准备导出）；失败项修复后全部定向复验，另复验相关神器／激活／估值及完整Mogaminator组，最终有效1,735项通过。内容172项（初测170项、两项修复复验）、导入198、本地化39、协议7、契约单元7、保存容器2、回放9、Tauri原生层24、前端226及审计工具8项通过。103个创角构筑的生成审计、295例源C评分与351项适用实例、相关Clippy、格式、类型、生成绑定／Schema及source/lock检查通过。

合并保留体内装置校验与恢复、装备激活／充能限制、宝箱保存、钓鱼和待召回休息。固定神器的自动拾取匹配名补入真实底材，修复『青龙元素』中英文默认规则不一致；显示名不变。旧测试补齐穿戴与资源上限刷新，Razorback替代专项预先标记瞬时神器已生成以隔离随机神器分支；基础池断言同步大木箱A15/2、无普通分配的钓竿／大号角及新增骑乘武器。号角走原版基础价格路径，从不适用的装备C评分实例清单排除，未用零分替代实际价格。协议1.263、payload25、State Hash Schema130表达联合格式；26条契约的41处差异全部为状态／保存回环哈希，事件与行为断言不变，刷新为contract-v330后全部通过。各方向原有桌面证据保留原范围，本轮没有重新构建可玩产物或运行桌面E2E／Android。
...
当前 103 个开放构筑的生成审计覆盖来源条件与每构筑五个范围：[审计输入](../design/generation-build-applicability.json)记录实际入口、实现和测试引用，[生成矩阵](../design/ego-contract-audit.json)由完整来源审计生成并接受只读 CI 检查。本轮合入食魔者、C4／C5 神器、装置间充能及阿斯加德的来源与消费者记录；职业限制、奖励与重复神器替代沿用实际规则。当前可达的文档证据缺口为零；缺失身份／内容、重铸入口和对象表示限制仍单列，全原版完成标记为 false。审计引用存在不代表执行过测试，本轮实际结果见上文。后续按[内容开发](content-development.md#职业与领域-build-的生成接入)维护审计，交接按[并行协作](parallel-development.md#职业与领域生成审计的交接)执行。
