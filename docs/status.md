...
| 最近验证内容包 | 1.429.0（B1／B2 源内容待 B7 统一更新版本／lock） | [pack](../packs/rfb-demo-original/pack.json)、[lock](../packs/rfb-demo-original/content.lock.json) |
...
正式源目录含 11 个 Class、77 个 Build、57 个 Race、36 本能力书、1,903 个 ability 文件、497 个 item、1,407 个 actor、169 个 affix、152 个 mutation。世界定义含 33 个 dungeon 条目；城镇源目录有 6 个 town、60 个 shop、64 个 townFacility。这些是定义/源文件数量，不是完整规则或已验收内容数量。

A1–A10 新增三十八件固定神器与五种底材；连同游侠两把弓与阿蒙专属神器，再加已有底材护具首组 132／238／357／380，以及四种身体护甲底材和神器 20／235／236／242，B1 又实现四种长柄底材及五件神器，B2 再实现三种钝器底材和四件神器，当前定义映射为 103/392（B1／B2 九件未验证）。B0 的普通获取／装备／保存、源估值和守卫替代奖励等 183 项相关核心、27 项内容、39 项本地化测试，以及 70 构筑审计、相关 Clippy 和 26 条未刷新的契约通过。显式守卫替代表在神器模式下已接入随机神器工厂，曼洞及拉莱耶回归通过。A10 的 Greater Hell-Beast 192 专属奖励保留零稀有度与已生成时跳过的源规则。法术道具方向的 235 项核心、36 项内容、39 项本地化、66 构筑审计与 8 项工具测试、相关 Clippy 和 26 条未刷新契约为该批证据；合并时的基线数量曾由 `rfb-contentc inspect-source` 核对，B1／B2 的新源内容尚未编译，集成检查见下文。范围与保留缺口见[神器计划](artifact-import-plan.md#a10-统一验证结果)：145／322 仅当前身份普通分支，萨鲁曼额外掉落、未开放身份／领域／地点消费者仍保留。没有新增自然练级、桌面或 Android 验收。
...
**B1／B2 实现待验证：**B1 新增普通底材 93／98／104／106 和神器 152／227／101／105／326，固定眩晕与斩击共享消费者及其测试已补；B2 再补底材 133／134／136 和神器 157／194／113／121，复用亡灵杀伤、品牌、吸血、禁传送与骑乘消费者，并写好对应测试。普通分配均已写入；194 的额外 33% 怪物掉落仍开放。按用户约定，B1–B7 期间不做编译验证，B7 完成后统一处理测试、C 估值、生成审计、包版本／lock 和契约。当前源内容领先于最近验证的 1.429.0；下列既有通过记录不覆盖 B1／B2。

当前固定神器定义为 103/392（含 B1／B2 九件待验证定义），逐项清单已合并游侠两把弓、阿蒙专属神器及两组共八件普通护具。『恐惧面具』铁盔的生成身份分支已接入：战士、骑兵、狂战士获得源随机能力／抗性，其他当前职业获得固有诅咒；法师交叉测试覆盖读档后反魔法实际拒绝施法、不消耗法力／RNG，以及卸下后法力上限和施法恢复。[神器计划 A0–A10](artifact-import-plan.md)已完成本轮约定范围，未开放分支与桌面验收继续单列。定义数量不代表自然获取或完整玩法均已验收，边界见[物品计划 I3](remaining-item-coverage-plan.md#i3固定神器与神器创造入口)。
