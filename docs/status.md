...
核对日期：2026-09-11；法术道具分支基于 `253c9ee5a` 新增 I3 高等抗性装备五件，保留奥林匹斯 OL1–OL4、霍比特人装备、I4 毒针、I5 工艺和 I6 堆叠首组。此处为独立分支集成快照，未同步 main 的后续 Mage 批次。本页记录代码/配置事实和注明范围的验收证据。当前数值以链接的源文件为准。
...
| 内容包 | 1.419.0 | [pack](../packs/rfb-demo-original/pack.json)、[lock](../packs/rfb-demo-original/content.lock.json) |
...
正式源目录含 9 个 Class、17 个 Build、57 个 Race、36 本能力书、1,901 个 ability 文件、417 个 item、1,405 个 actor、169 个 affix、152 个 mutation。世界定义含 32 个 dungeon 条目；城镇源目录有 6 个 town、60 个 shop、63 个 townFacility。这些是定义/源文件数量，不是完整规则或已验收内容数量。
...
I3 高等抗性第四组已接入固定神器 21/24/27/28/36，正式映射为 40/392。五件复用普通生成与现有固定神器物化路径，增加共用 XTRA_H_RES 配置；按原版从 12 项抽一次，已有抗性不排除、不重抽，重复项不叠加。核心验证完整池受控生成、未知状态保存、鉴定揭示、装备属性/抗性、随机结果与唯一性保存及后续生成/RNG。78 项物品测试、源 C 静态估值、固定神器内容/名称/锁检查、十构筑审计及 26 条契约通过，未刷新 fixture。来源及各组边界见[物品计划 I3](remaining-item-coverage-plan.md#i3固定神器与神器创造入口)。本组未执行桌面或 Android 验收。
