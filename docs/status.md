...
核对日期：2026-09-11。当前基于种族职业主线 `080005368`、法术道具 `ea723f342` 与地牢城镇 `bd94fe086` 的集成，另完成法师出生、双领域学习成长、副领域改换保存、职业生成/奖励关联及正式 UI。本页记录代码/配置事实和注明范围的验收证据。当前数值以链接的源文件为准。
...
| 法师（八领域双选） | `demo.build.mage-<first>-<second>`，主副不可相同 | 56 个有序组合已开放；双书学习、重复研习、遗忘恢复、副领域改换与 UI/保存专项通过，自然游玩及优化版交付待第七步 |
