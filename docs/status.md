...
| 内容包 | 1.423.0；A1–A10 已统一编译验证并同步 lock | [pack](../packs/rfb-demo-original/pack.json)、[lock](../packs/rfb-demo-original/content.lock.json)、[本轮约定](artifact-import-plan.md) |
...
正式源目录含 10 个 Class、73 个 Build、57 个 Race、36 本能力书、1,902 个 ability 文件、466 个 item、1,406 个 actor、169 个 affix、152 个 mutation。世界定义含 32 个 dungeon 条目；城镇源目录有 6 个 town、60 个 shop、64 个 townFacility。这些是定义/源文件数量，不是完整规则或已验收内容数量。A1–A10 新增三十八件固定神器与五种底材，共四十三个定义；固定神器映射为 83/392，剩余 309。A10 接入 192 及底材 245，以正式兽穴入口的 Greater Hell-Beast 死亡必掉奖励验证拾取、装备和读档唯一性；按源已生成时跳过，不生成重复替代品。
...
职业与种族目录在 [character-creation.ts](../web/src/character-creation.ts)，提交入口在 [session-shell.ts](../web/src/session-shell.ts)，表单在 [web/index.html](../web/index.html)。当前入口提供 66 个构筑、46 个种族，其中法师占 56 个有序领域组合：
