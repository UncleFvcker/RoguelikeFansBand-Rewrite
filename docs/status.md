...
核对日期：2026-09-12。当前物品分支已完成B7统一验证；最近集成包含种族职业 `edfcac5fb`、法术道具 `886d8da89` 和地牢城镇 `1ef1b7261`，合入游侠、A1–A10 神器与金字塔土丘。本页区分当前代码/配置与各批注明范围的验收证据。
...
| 最近验证内容包 | 1.430.0 | [pack](../packs/rfb-demo-original/pack.json)、[lock](../packs/rfb-demo-original/content.lock.json) |
...
正式源目录含 11 个 Class、77 个 Build、57 个 Race、36 本能力书、1,903 个 ability 文件、517 个 item、1,407 个 actor、169 个 affix、152 个 mutation。世界定义含 33 个 dungeon 条目；城镇源目录有 6 个 town、60 个 shop、64 个 townFacility。这些是定义/源文件数量，不是完整规则或已验收内容数量。

B1–B7 共16种普通底材、20件固定神器的普通生成分支已接入并完成统一核心验证。B7 新增101长柄大镰刀与109『阿瓦维尔』、102伊昂威的巨斧。阿瓦维尔使用15–35回合召回、1000 tick冷却；伊昂威使用强度200、范围20的群体灭绝、10000 tick冷却。共享灭绝按源距离、任务层／独特怪物／坐骑保护、抵抗后觉醒与持久免疫、逐目标疲劳及不致死下限执行；普通灭绝与湮灭死灵各自保留源德行变化。 当前正式固定神器定义映射114/392；数量不代表全部原版消费者或专属入口均已完成。范围与保留依赖见[神器计划](artifact-import-plan.md#第二步普通底材与依赖神器分组)。
...
C1已接入143波罗莫的护身符、45精灵披风『霍尔科莱斯』、112连枷『托提拉』和356图伯的长弓，正式物品521项、固定神器定义118/392，内容包／lock为1.431.0。四种群体控制按源视线／可投射路径、强度、免疫与豁免、初始及重复计数执行；39／41和独立Ego／随机神器激活共用此路径。图伯弓按源规则无法射中字符B的怪物，仍消耗正常命中检定与弹药。新角色核心验证覆盖获取／装备／激活、冷却中保存与恢复、后续RNG和真实射击；143来自完整普通池，续组稀有余项按实际取得底材定向验证。本批283项核心检查通过（既有Ego的2项ignored保留），包括物品／控制／Ego／随机神器／射击回归；内容161项、导入196项、本地化39项、源C估值、Schema、70构筑审计、Clippy及26条active契约通过，未刷新fixture。恢复仍使用现有world tick；源噪声唤醒、恐惧／混乱自然恢复及定制复合控制适配未整体重写。参数和边界见[神器计划C1](artifact-import-plan.md#各组行为验收)，本批不含桌面或Android验收。
