...
游侠已完成[七步接入计划](ranger-class-plan.md)前六步：自然主领域＋咒术/死亡/奥秘/恶魔四个 Build、160项参数、出生/成长、3级MP与80基础学习容量已接入；随机学习共享支出、1600/1400主副练习、遗忘恢复、失败/美德、副领域改换与保存续演已实现。射击技能/箭与非箭射速、额外射速和重弓/Tomte边界、树林通行及15级探测MP→HP已通过实际动作与保存验证；合法坐骑树格保存现沿实际移动规则校验。生成偏好、神器卷轴职业分支、两件固定弓与1:4旧城堡奖励/重复替代、盗贼长剑、五个弓手公会Owner和两塔Member已接入；当前领域书本偏好、自动铭刻及设施角色随改换更新。编译器保留源书内顺序；[来源审计](ranger-source-audit.md)区分实际完成与保留适配，四个 Build 的20项生成记录和报告已正式登记，当前共70个创角入口。游侠位于“箭术 → 游侠 → 第二领域”，书本页显示随机学习、当前/历史领域、容量、熟练度、遗忘与确认后立即尝试学习的后果。Tauri standalone 中英文四组合创角、1—3级说明、显式15级准备、原生键盘/焦点、取消/忙碌锁、待确认和改换后保存导入、390px及200%专项通过，证据在 `test-results/ranger-ui/`。第七步的新游戏实战闭环和优化桌面交付尚未完成。工艺不属于原版游侠候选，混沌/王牌留给后续领域批次。
