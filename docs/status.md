...
阿斯加德：[AS1–AS7 计划](../design/asgard-dungeon-plan-20260912.md)已完成 AS1 来源核对及 AS2–AS4 实现。十五件固定神器、钓鱼竿／大号角／大木箱已有定义；神器使用链、十三位神祇死亡绑定、弗雷／弗蕾亚选择掉落、埃吉尔烈酒、奥丁召来维达、弗丽嘉独立宝箱、开箱／拆除／陷阱与保存已有 Rust 消费者及所需前端操作。普通神器资格保留六件可抽取、九件 QUESTITEM 排除；64–88 共 25 个深度已定义，地图复用 CAVE／CAVERN、混合墙与矿脉；四层／两层竖井、实际保存回程及同源金字塔土丘连接已修正。已登记地图所需神系及分配引用，荒野入口放置、守卫、征服奖励与完整生态验收仍待 AS5–AS6。专项测试代码已写，未执行。按用户要求，中途不编译、不运行测试，AS7 实现与来源收口结束后统一生成并验证；当前不能视为编译通过或实际验收完成。
...
| 内容包源码 | 1.440.0；lock 暂留已验证的 1.437.0，待 AS7 编译生成 | [pack](../packs/rfb-demo-original/pack.json)、[lock](../packs/rfb-demo-original/content.lock.json) |
...
正式源目录含 13 个 Class、109 个 Build、57 个 Race、36 本能力书、1,907 个 ability、567 个 item、1,409 个 actor、169 个 affix、152 个 mutation。世界定义含 34 个 dungeon（含入口尚未放置的阿斯加德）；城镇有 7 个 town、69 个 shop、67 个 townFacility。基础分配池为 408 行，固定神器定义映射 149/392；创角开放 102 个 Build。AS2–AS4 新增定义的验证及大木箱 A15/2 分配同步尚待 AS7；这些是定义与入口数量，行为验收范围见下文。
