...
祖尔导入：[Z1–Z6 计划](../design/zul-town-import-plan-20260912.md)中的 Z1–Z2 已实施、待验证。已注册 (77,6) 的 94×57 山地模板与九家独立商店，保留 2,158 个显式格、3,200 个荒野继承格及三塔门格；祖尔不使用普通城镇的刷怪排除。珠宝店与龙皮百货已接随机装备生成、专属估值／交易规则和库存保存；补十种龙鳞甲及喷吐激活，14 种源龙鳞甲与五种龙皮部位均有正式定义，高等级源门槛保持。没有旅店／Home／博物馆，物理到访尚不开放祖尔传送。Z3–Z6 的三塔、任务与验收待推进；按用户安排，全部实施后统一编译和测试，回归用例已编写但未运行，Schema／绑定／分配审计／内容锁待生成，尚无本批运行证据或桌面产物。店主固定、维护与生成器保留适配见计划 Z2。
...
| 协议 | 源 1.254；生成绑定待统一更新 | [协议常量](../crates/rfb-protocol/src/lib.rs) |
...
| 内容包 | 源 1.432.0；lock 1.430.0 待统一编译更新 | [pack](../packs/rfb-demo-original/pack.json)、[lock](../packs/rfb-demo-original/content.lock.json) |
...
正式源目录含 11 个 Class、77 个 Build、57 个 Race、36 本能力书、1,903 个 ability 文件、479 个 item、1,407 个 actor、169 个 affix、152 个 mutation。世界定义含 33 个 dungeon 条目；城镇源目录有 7 个 town、69 个 shop、64 个 townFacility。这些是定义/源文件数量，不是完整规则或已验收内容数量；新增祖尔尚待统一验证。
