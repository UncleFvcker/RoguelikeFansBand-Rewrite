...
QUESTITEM Q2：十五件正式内容及现有命名怪物掉落已接入，定义累计45/83，余38件。震地者手套IMPACT补入所持武器近战链，百眼／孔德里激活及杰克无燃料4格照明复用现有消费者。维克那／孔德里保留既有守卫奖励，墨伊蕾保留海洋荒野入口；未开放身份分支仍列条件。源参数和核心回归用例已写，本批未执行编译或验证；Q1／Q2完整验收、lock及源C估值／审计更新统一待Q5。下一批Q3a。
...
| 内容包 | 正式包1.458.0；lock仍为1.451.0，按用户要求待Q5统一生成验证 | [pack](../packs/rfb-demo-original/pack.json)、[lock](../packs/rfb-demo-original/content.lock.json) |
...
正式源目录含 14 个 Class、110 个 Build、57 个 Race、36 本能力书、1,915 个 ability、609 个 item、1,410 个 actor、169 个 affix、152 个 mutation。世界定义含 38 个 dungeon（四类随机地牢及北欧神系门控的阿斯加德）；城镇有 7 个 town、69 个 shop、67 个 townFacility。基础分配池为 424 行，固定神器定义映射 175/392；创角开放14职业、103个Build。这些是定义与入口数量，行为验收范围见下文。
