...
阿斯加德：[AS1–AS7 计划](../design/asgard-dungeon-plan-20260912.md)已完成 AS1 来源核对、AS2–AS5 实现及 AS6 验收准备。十五件固定神器、钓鱼竿／大号角／大木箱已有定义；神器使用链、十三位神祇死亡绑定、选择掉落、烈酒、维达后继、独立宝箱及开箱／拆除／陷阱与保存已有 Rust 消费者及所需前端操作。普通神器资格保留六件可抽取、九件 QUESTITEM 排除；64–88 共 25 个深度已定义，地图复用 CAVE／CAVERN、混合墙与矿脉，四层／两层竖井及实际保存回程已接通，同源金字塔土丘连接已修正。正式荒野 (94,11) 入口、海姆达尔／奥丁与一次征服卷轴已绑定，神系门控沿既有路径；唯一守卫域外死亡不复生，末层保存校验与全局额度一致。诗歌蜜酒的当地普通分配、源显示名、使用时长／上限及驱散已接通；Good／Great 排除，不能由获取卷轴或大木箱自然生成。AS6 已编写正式入口、三场完整 HP 近战、九深度竖井、奖励使用、返回／召回及保存恢复的核心流程和桌面场景；清场保留实际维达后继，普通 EXE 拒绝准备接口的检查已就绪。专项与桌面脚本均未执行，尚无本批报告／截图。按用户要求，中途不编译、不运行测试，AS7 实现与来源收口结束后统一生成并验证；当前不能视为编译通过或实际验收完成。
...
| 协议源码 | 1.260；绑定待 AS7 | [协议常量](../crates/rfb-protocol/src/lib.rs) |
| State Hash Schema | 129；契约待 AS7 | [核心常量](../crates/rfb-core/src/game/mod.rs) |
| save header / payload / 容器 | 14 / 24 / 1 | [协议常量](../crates/rfb-protocol/src/lib.rs)、[rfb-save](../crates/rfb-save/src/lib.rs) |
| 内容包源码 | 1.441.0；lock 暂留已验证的 1.437.0，待 AS7 编译生成 | [pack](../packs/rfb-demo-original/pack.json)、[lock](../packs/rfb-demo-original/content.lock.json) |
...
正式源目录含 13 个 Class、109 个 Build、57 个 Race、36 本能力书、1,907 个 ability、567 个 item、1,409 个 actor、169 个 affix、152 个 mutation。世界定义含 34 个 dungeon（阿斯加德入口已放置，受出生神系门控）；城镇有 7 个 town、69 个 shop、67 个 townFacility。基础分配池为 408 行，固定神器定义映射 149/392；创角开放 102 个 Build。AS2–AS5 新增定义的验证及大木箱 A15/2 分配同步尚待 AS7；这些是定义与入口数量，行为验收范围见下文。
...
阿斯加德 AS4 已按同源规则纠正金字塔土丘连接：64→68→72→76→80→82→84→86→88→90→92，共十一深度；阈值附近保存实际回程，65–67 的向上竖井回地表。上段 PM 两层路线属于旧配置证据，相关往返／边界用例已更新，尚待 AS7 统一验证。
