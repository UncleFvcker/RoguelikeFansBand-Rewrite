...
本轮集成的核心全量检查有 1,354 项通过，3 项失败经修正后在决斗者 37 项、神器卷轴 5 项及职业偏好专项中全部通过，另有 3 项原有 ignored。修正包括卷轴退款时保留冷却、已装备神器生成后更新 HP/MP 上限、决斗者量身武器复用熟练度偏好；测试明确传送场地照明与成功判定。内容 155、导入器 193、本地化 39、协议 7、保存 2、回放 9、Tauri 原生层 23、相关前端 93、生成审计工具 8 项通过。26 条 active 契约经观察后更新合并状态哈希及两处 RNG 计数并通过；最终全部非哈希断言与 `ea723f342` 一致，保留该方向已验证的装置随机序列变化及采矿成功种子。相关 Clippy、绑定/Schema、内容锁、类型和格式检查通过；详细命令见合并提交说明。各方向已有 standalone 交付及桌面证据保留各自范围，本轮未重建 standalone 或运行桌面 E2E/Android。
...
当前十构筑的生成审计覆盖 36 条来源条件与每构筑五个范围：[审计输入](../design/generation-build-applicability.json)记录实际入口、实现和测试引用，[生成矩阵](../design/ego-contract-audit.json)由完整来源审计生成并接受只读 CI 检查。本次将决斗者纳入，复用基础熟练度偏好补齐量身武器筛选；同时保留盾牌与 200 重量护甲限制、旧城堡奖励及神器替代。真实新游戏用例覆盖量身生成、装备、负向神器、保存和后续 RNG；创造神器卷轴的实际指令覆盖八个可读卷轴构筑，狂战士继续按职业规则拒绝。十构筑当前可达证据缺口为零；缺失身份/内容、重铸入口和对象表示限制仍单列，全原版完成标记为 false。审计引用存在不代表执行过测试，本轮实际结果见上文验收记录。后续按[内容开发](content-development.md#职业与领域-build-的生成接入)维护审计，交接按[并行协作](parallel-development.md#职业与领域生成审计的交接)执行。
