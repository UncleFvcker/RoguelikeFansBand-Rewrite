...
核对日期：2026-09-11；法术道具分支基于 `e90ffc2f8` 新增 I3 普通武器两件，保留奥林匹斯 OL1–OL4、霍比特人装备、I4 毒针、I5 工艺和 I6 堆叠首组。此处为独立分支集成快照，未同步 main 的后续 Mage 批次。本页记录代码/配置事实和注明范围的验收证据。当前数值以链接的源文件为准。
...
| 内容包 | 1.418.0 | [pack](../packs/rfb-demo-original/pack.json)、[lock](../packs/rfb-demo-original/content.lock.json) |
...
正式源目录含 9 个 Class、17 个 Build、57 个 Race、36 本能力书、1,901 个 ability 文件、412 个 item、1,405 个 actor、168 个 affix、152 个 mutation。世界定义含 32 个 dungeon 条目；城镇源目录有 6 个 town、60 个 shop、63 个 townFacility。这些是定义/源文件数量，不是完整规则或已验收内容数量。
...
I3 普通武器第三组已接入 79『贡德瑞坎』弯刀和 86『弗拉斯吉尔』细剑；连同既有普通护具、霍比特人及奥林匹斯神器，正式映射为 35/392。复用普通固定神器生成和装备/战斗消费者，核心验证完整池受控生成、拾取/鉴定/装备、属性与抗性、实际攻击和寒冷品牌/动物杀伤、保存后回血/漂浮越坑、唯一性及后续生成/RNG。76 项物品测试、来源 C 估值、固定神器内容/名称/锁检查、十构筑审计及 26 条契约通过，未刷新 fixture。装备再生沿用每 10 tick 回复 1 HP 的公共适配，与原版百分比再生仍有差异；来源及证据范围见[物品计划 I3](remaining-item-coverage-plan.md#i3固定神器与神器创造入口)。本组未执行桌面或 Android 验收。
