...
| 协议源码 | 1.260；绑定已同步 | [协议常量](../crates/rfb-protocol/src/lib.rs) |
| State Hash Schema | 129；26 条 active 契约已复验 | [核心常量](../crates/rfb-core/src/game/mod.rs) |
| save header / payload / 容器 | 14 / 24 / 1 | [协议常量](../crates/rfb-protocol/src/lib.rs)、[rfb-save](../crates/rfb-save/src/lib.rs) |
| 内容包源码 | 1.442.0；lock 与来源检查一致 | [pack](../packs/rfb-demo-original/pack.json)、[lock](../packs/rfb-demo-original/content.lock.json) |
...
正式源目录含 13 个 Class、109 个 Build、57 个 Race、36 本能力书、1,907 个 ability、567 个 item、1,409 个 actor、169 个 affix、152 个 mutation。世界定义含 34 个 dungeon（阿斯加德入口已放置，受出生神系门控）；城镇有 7 个 town、69 个 shop、67 个 townFacility。基础分配池为 409 行，固定神器定义映射 149/392；创角开放 102 个 Build。AS2–AS5 新增定义已通过内容检查，大木箱 A15/2 分配与来源审计已同步；这些是定义与入口数量，行为验收范围见下文。
...
C3已接入271『弩炮』、324『穿刺者』、243『嗜血裂杀』，当前正式物品532项、固定神器定义129/392，包／lock为1.433.0。装备激活调用真实穿透射击、骑乘冲锋和相邻逐次近战；补齐射击费用与集中力时序、决斗者冲锋锁定和嗜血裂杀每手结束的50%流血反噬。弩炮来自完整普通池，续两件按真实底材与完整神器门槛条件化生成；覆盖鉴定／装备、动作结果、取消／失败、弹药／冷却、死亡中断及保存续演。Blood-Knight必定反噬与Ninja特殊冲锋等待身份开放，流血自然恢复等共享适配保留；未新增自然练级、桌面或Android验收。来源与范围见[神器计划C3](artifact-import-plan.md#各组行为验收)。

C3共318项不同核心测试、161项内容及39项本地化测试通过，包含一次旧骑枪测试前置修正后的定向复验；覆盖六项新增行为、物品全组、狙击手、骑枪Ego、狂战士、决斗者、高阶法师、装置及战斗／估值消费者。295例源C估值、来源／lock、生成Schema、70构筑适用性、26条active契约、Clippy与格式检查通过，无fixture刷新。下一组按龙鳞甲交接状态推进C4或C5。
