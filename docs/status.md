...
[竞技场地牢 AR1](../design/arena-dungeon-plan-20260910.md#ar1圆形房间墙与通道) 已完成生成能力，正式入口仍未开放。`arena-rooms` / `circle` 支持半径 3–7 的圆形房间、永久墙中的生成通道、按深度选择通道与房间／外墙照明；普通门从真实直通道落位，内容校验排除 vault、streamer 和其他地形布局。以 96×33、6 房间配置验证可复现性、变化、连通、玩家／楼梯／守卫预留和永久墙不可普通挖掘；沿用现有预算、通道与单门适配，不宣称复刻原版尺寸、门数量或 RNG 序列。104 项相关核心测试、1 项内容边界测试及 26 条 active 契约通过；未修改正式包内容、协议或保存格式，未刷新 fixture。怪物专用分配、创造神器卷轴和 31 层正式接入留给 AR2–AR4。
