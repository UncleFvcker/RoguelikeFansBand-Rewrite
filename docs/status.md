...
| 内容包 | 1.439.0 | [pack](../packs/rfb-demo-original/pack.json)、[lock](../packs/rfb-demo-original/content.lock.json) |
...
正式源目录含 13 个 Class、109 个 Build、57 个 Race、36 本能力书、1,907 个 ability、552 个 item、1,409 个 actor、169 个 affix、152 个 mutation。世界定义含 33 个 dungeon；城镇有 7 个 town、69 个 shop、67 个 townFacility。基础分配池为 408 行，固定神器映射 137/392；创角开放 102 个 Build。这些是定义与入口数量，行为验收范围见下文。
...
C3共318项不同核心测试、161项内容及39项本地化测试通过，包含一次旧骑枪测试前置修正后的定向复验；覆盖六项新增行为、物品全组、狙击手、骑枪Ego、狂战士、决斗者、高阶法师、装置及战斗／估值消费者。295例源C估值、来源／lock、生成Schema、70构筑适用性、26条active契约、Clippy与格式检查通过，无fixture刷新。C4前置已随祖尔Z6合入；新的[排期](artifact-import-plan.md#排期与首组)为C4a（201→332）、C4b（130）、C4c（226）、C5a（14）、C5b（13），C4a／C4b已完成普通入口与核心验收，下一批为C4c。
