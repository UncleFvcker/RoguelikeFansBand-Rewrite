...
第一至四步已完成，核对日期 2026-09-10。对应[职业计划](berserker-class-plan.md)。下文保留第一步的来源与差异分析；第二、三步已实现范围及复核修正在第 8、9 节。第四步已开放“近战 → 狂战士”并通过能力、物品提示与布局专项；第五步的完整桌面试玩和产物交付验收待完成，当前范围见[状态页](status.md)。
...
| `races_a.c:1180`；`races_k.c:1191` | 龙人变形天然攻击等级乘 170% 已接入；吸血鬼不发黑暗卷轴。第二步复核确认当前吸血鬼只支持后天转化，没有出生消费者，卷轴例外留待该种族出生开放 |
...
| 出生合并 | 原版先熟练度、性格出生、职业出生、种族出生/默认食物与光源；`py_birth_obj` 完整鉴定、优先空槽，防重复装备，Android 不给身体护甲。本项目为食物/光源后 race→class→personality→build，沿用当前初始化顺序与单源 RNG；验证装备槽冲突和正式出生知识，不改所有既有种族的出生顺序。第二步复核：吸血鬼当前未开放出生 |
...
| 吸血鬼 | `_vampire_birth` 对狂战士跳过黑暗卷轴，连 2–5 数量 RNG 也不抽。第二步复核：`rfb-legacy.race.vampire` 仅用于后天转化，无 `startingItems` 或正常出生入口；当前不存在可过滤的卷轴。以后开放出生时接该例外，不为本职业新增吸血鬼入口 |
...
正式包 1.405.0 新增 `demo.class.berserker`、`demo.build.berserker`、对应技能集和玩家 actor。规则复用现有职业/装备/能力消费者：无 MP 出生与成长、负技能、熟练度与美德、等级被动、派生常驻狂暴、武器攻次与双持、穿无敌与击杀耗时、撞门/看破隐形、强感知、手动与自动物品限制、诅咒强拆、突变过滤、龙人变形倍率及五座公会会员均已接通。没有添加永久狂暴计时器、第二 HP 池或新持久字段。

Class HP 费用在成功效果或普通失败后支付；恰好等费可尝试，0 HP 仍存活，效果已致死则不二次扣费。职业失败率支持震慑，基础 0% 保持零；归还术所需的敏捷判定和纯 HP 费用已有字段可表达。本步以测试能力验证治疗后扣费、不足时不掷 RNG、失败扣至零；六项正式能力的取消、回血/自伤/死亡与保存继续由第三步用真实效果验收。
