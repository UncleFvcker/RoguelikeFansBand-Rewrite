...
审计日期：2026-09-11，初次审计对应[游侠计划](ranger-class-plan.md)第一步、基线 `6f9b22c1a6649833bf2ff975878efb8cedb235d3`。当前七步已完成：Class/四 Build、出生/成长、双领域随机学习/改换保存、射击/树林/探测、生成/奖励/公会、正式创角/UI及新游戏实战与优化桌面交付；实施、验收与适配边界见第8—13节。内容包1.425.0、协议1.253、State Hash Schema125、save header/payload14/20；本轮未改变保存格式。
...
`node e2e/tauri.e2e.mjs --ranger-play --fast-entry` 从正式 UI 创建 seed 925 的普通人类自然/咒术游侠。装备出生火把，以出生短弓和箭进入真实生成的兽穴，射击命中昆虫群并存活。按用户要求增加的 `--fast-entry` 只跳过城镇步行，将角色放到实际楼梯再正常下楼；不授予经验、装备或替换地牢生成。完整步行也已到达兽穴并完成出生弓箭命中，保留该阶段的 `natural-bow-start.rfbsave`；最终全流程报告使用快速入口，不能宣称两种走法的 RNG 起点相同。

3级和5级分别显式授予经验、照明、清理怪物并补满HP/MP；使用出生双方真实书本随机学习并实际施放。3级只够支付一次共享学习，5级再学习副领域，没有人为增加WIS或学习容量。15级探测先成功支付20MP，下一次自然失败仍支付9MP＋11HP；读取实际施放事件，不把职业能力投影中的书本练习计数当成成功证据。共享消息格式化器现优先使用核心投影的法术名称键，修复该职业能力失败时显示“未知实体”的问题。

50级准备包含两册第四阶书、以当前位置为中心的3×3地面/东侧树格及西侧源定义绵羊。实际进入树格、菜单导出、原生加载并离开树格，随后实际探测绵羊；高阶书随机学习保留源等级资格，三项咒术的等级要求超过50而不可学习/施放。改换副领域为死亡时立即随机学习，保留自然主领域练习和咒术历史；菜单导出后恢复并执行相同两次施放及下一次死亡随机学习，完整状态哈希和随机结果完全一致。没有替换成功率、RNG或熟练度，没有自然练到50级的声明。

报告、截图和明确标注准备内容的存档位于 `test-results/ranger-play/`。快速入口复用 `prepare_stairs_e2e`，核心检查覆盖无效位置拒绝、出生状态/回合/RNG不变、保存恢复和正常下楼生成；法师、决斗者、狂战士亦完成 `--fast-entry` 实战回归。命令见[验证指南](testing.md#桌面-e2e-快速到地牢)。第六步中英文四组合、390px/200%及键盘/焦点证据保留其 `22af452a5` 验收范围。

本里程碑 workspace（除Tauri）、Tauri原生层、全量前端、完整active契约、相关Clippy、类型/绑定/Schema、内容锁和70入口审计通过。没有修改内容包、协议、save/hash版本或刷新fixture。优化EXE通过进程定向的Windows UI Automation控件模式完成普通人类自然/咒术游侠创角、1级MP/容量投影及正常退出，没有使用WebDriver或测试经验命令；报告在 `test-results/ranger-optimized/checks.json`。此烟测与同源WebDriver实战分开记录，产物包含源码快照、许可、校验值与证据，交付路径见状态页和交付包README。Android、自然高等级成长、完整通关及前文保留的原版适配边界不在本批验收声明内。
